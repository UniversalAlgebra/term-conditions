...
%% \address{Department of Mathematics, University of Colorado, Boulder 80309 USA}
...
%% \urladdr{http://math.hawaii.edu/~ralph}
%
%  (Add a similar block for other authors)
%
%   Title and author both have running head options:
%
\title[Complexity of Difference Term Existence]{On the Complexity of Difference Term Existence}
...
\keyword{difference term}
\keyword{finite idempotent algebra}
\keyword{efficient algorithm}

\subject{primary}{msc2000}{08B05}
\subject{secondary}{msc2000}{08B10}
\subject{secondary}{msc2000}{68Q25} 
\subject{secondary}{msc2000}{03C05}
...
\newtheorem{thm}{Theorem}[section]    % Theorem environment with numbering
...
Before proceeding, we need to agree on some more notational conventions.
If $\alpha \in \Con(\bA)$ and $\beta \in \Con(\bB)$, then
$\alpha \mytimes \beta$ denotes the set of pairs $((a,b),(a',b'))\in (A\times B)^2$ satisfying
...
\ifthenelse{\boolean{extralong}}{
  ...
  \begin{proof}
    By monotonicity of the commutator, $\bigl[\Meet \alpha_i, \Meet \beta_i\bigr] \leq
    \comm {\alpha_i} {\beta_i} \leq \bigl[\Join \alpha_i, \Join \beta_i\bigr]$,
    for all $i \in I$.
  \end{proof}
}{}
