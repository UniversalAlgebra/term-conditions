...
\subsection{Two inductive stages}
...
Define $\ell := 2n^2-1$ and let
\[
  \{(a_0, b_0, 0), (a_0, b_0, 1), (a_1, b_1, 0),(a_1, b_1, 1), \dots,
  (a_{\ell-1}, b_{\ell-1},0), (a_{\ell}, b_{\ell},1)\},
\]
be an enumeration of the entire set $A^2 \times \{0,1\}$.
The first inductive stage of our procedure computes the Cayley table of a \ldto for 
the set $S_\ell := \{(a_0, b_0, 1), (a_1, b_1, 1), \dots,
(a_{\ell-1}, b_{\ell-1},1), (a_{\ell}, b_{\ell},0)\}$.
...
$d(x,y,z) := q(p(x,y,z), p(y,y,z), z)$.
As we saw in the proof of Theorem~\ref{thm:local-diff-terms}, this is a 
\ldto for $A^2 \times \{0,1\}$, hence a difference term operation for $\alg{A}$.

We now describe the two inductive stages in more detail and show that each can be completed in 
$n^2$ calls to Algorithm~\ref{alg:ld-2}.
Define 
\begin{align*} 
S_1 &:= \{(a_0, b_0, 1),(a_{\ell}, b_{\ell},0)\},\\
S_2 &:= \{(a_0, b_0, 1), (a_1, b_1, 1), (a_{\ell}, b_{\ell},0)\},\\
& \vdots\\
S_\ell &:= \{(a_0, b_0, 1), (a_1, b_1, 1), \dots,
(a_{\ell-1}, b_{\ell-1},1), (a_{\ell}, b_{\ell},0)\}.
\end{align*}
Each of the enumerated steps below requires a single call to Alg.~\ref{alg:ld-2}.
\begin{enumerate}[1.]
  \item Compute $p_1 \in \dtr S_1$ (i.e.~$p_1$ is a \ldto for $S_1$); % for $\{(a_0, b_0, 1), (a_{\ell}, b_{\ell},0)\}$.    
  ...
  form
  $p(x,y,z) = q_{\ell-1}(p_{\ell-1}(x,y,z), p_{\ell-1}(y,y,z), z) \in \dtr S_\ell$;
\end{enumerate}
...
The second inductive stage is similar. Define
\begin{align*} 
  T_1 &:= \{(a_0, b_0, 0), (p(a_{\ell}, a_{\ell}, b_{\ell}), b_{\ell}, 1)\},\\
  T_2 &:= \{(a_0, b_0, 0), (a_1, b_1, 0), (p(a_{\ell}, a_{\ell}, b_{\ell}), b_\ell, 1)\},\\ 
  & \vdots\\
  T_\ell &:= \{(a_0, b_0, 0), \dots,
(a_{\ell-1}, b_{\ell-1},0), (a_{\ell}, b_{\ell},0), (p(a_{\ell}, a_{\ell}, b_{\ell}), b_\ell, 1)\}.
\end{align*}
...
Each of the enumerated steps below requires a single call to Alg.~\ref{alg:ld-2}.
\begin{enumerate}[1.]
  \item Compute $p_1 \in \dtr T_1$ (i.e.~$p_1$ is a \ldto for $T_1$);
  \item compute $q_1\in \dtr \{(a_1, p_1(a_1, b_1, b_1), 0), (p(a_{\ell}, a_{\ell}, b_{\ell}), b_{\ell}, 1)\}$; \\
  form $p_2(x,y,z) = q_1(x, p_1(x,y,y), p_1(x,y,z)) \in \dtr (T_2\cap S_2)$;
  \item compute $q_2\in \dtr \{(a_2, p_2(a_2, b_2, b_2), 0), (p(a_{\ell}, a_{\ell}, b_{\ell}), b_{\ell}, 1)\}$; \\
  form $p_3(x,y,z) = q_2(x, p_2(x,y,y), p_2(x,y,z)) \in \dtr (T_3\cap S_3)$;\\
  $\vdots$
  \item[$\ell$.] compute $q_{\ell-1}\in \dtr \{(a_{\ell-1}, p_{\ell-1}(a_{\ell-1}, b_{\ell-1}, b_{\ell-1}),0), (p(a_{\ell},a_{\ell}, b_{\ell}),  b_{\ell},1)\}$; \\
  form
  $d(x,y,z) = q_{\ell-1}(x, p_{\ell-1}(x,y,y), p_{\ell-1}(x,y,z)) \in \dtr (T_\ell \cap S_\ell)$;
\end{enumerate}
...
\begin{algorithm}
  % \KwIn{$\tabT \eta_{\kk}$ and $\tabT \sA_{2k}$}
  \KwOut{$p \in \dtr (T_{\ell}\cap S_\ell)$}
  
  compute $p_1 \in \dtr T_1$. % for $\{(a_0, b_0, 1), (a_{\ell}, b_{\ell},0)\}$.    
  
  \For {$1\leq i < \ell$} {
    compute $q_i\in \dtr \{(a_i, p_i(a_i, b_i, b_i), 0), (p(a_{\ell}, a_{\ell}, b_{\ell}), b_{\ell}, 1)\}$;
  
    form $p_{i+1}(x,y,z) = q_i(x, p_i(x,y,y), p_i(x,y,z))$;
  }

  \Return $p_\ell$
  \caption{Compute a \ldto for $S_\ell$\label{alg:new-2-2}}
\end{algorithm}
