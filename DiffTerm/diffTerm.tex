...
$(0,1) \notin \tau'$. 
Since $(0,1) \in \sigma|_M\setminus \tau'|_M$, this 
...
We build up the algorithm in small steps in the following subsections, 
each of which demonstrates how to find a \ld term operation for a 
...
$t(x,y,z) = y$.
Moreover, if $a_0 = b_0$, then by idempotence every ternary term $t$ 
satisfies $t(a_0, b_0, b_0) = a_0$; thus the third projection is an \ld 
term for $\{(a_0,b_0,\chi_0), (a_1, b_1, \chi_1)\}$, 
...
using Subroutine 01, compute an \ld term $t$ for the set
\begin{equation*}
\{(a_0, b_0, 0), (a_1, b_1, 1)\};
\end{equation*}
\item \label{item:001-2} using Subroutine 01, compute an \ld term $s$  for the set
\begin{equation*}
...
First observe that
$d(a_0, b_0, b_0)=
s(t(a_0,b_0,b_0), t(b_0,b_0,b_0), b_0)$ is congruent to 
$s(a_0, b_0, b_0)$ modulo $\com{\theta_0}$, which is 
congruent to $a_0$ modulo $\comr{\theta_0}$.
Next,
% and the latter is congruent to $s(a_0, b_0, b_0)$ modulo
% $\com{\theta_0}$, which in turn is congruent to 
% $a_0$ modulo $\com{\theta_0}$. Next,
$d(a_1, a_1, b_1)
=s(t(a_1, a_1, b_1),t(a_1,a_1, b_1), b_1)$, which is equal to 
$s(b_1, b_1, b_1) = b_1$ because of the property satisfied 
in Step (\ref{item:001-1}) of the subroutine.
Finally,
$d(a_2, a_2, b_2)=s(t(a_2, a_2, b_2), t(a_2,a_2,b_2), b_2)$,
which is equal to $b_2$ because of the property satisfied in 
Step (\ref{item:001-2}) of the subroutine.
\end{proof}
