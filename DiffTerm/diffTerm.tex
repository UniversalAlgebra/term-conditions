...
Define $\ell := 2n^2-1$ and 
enumerate the set $A^2 \times \{0,1\}$ as follows:
\[
  \{(a_0, b_0, 0), (a_0, b_0, 1), (a_1, b_1, 0),(a_1, b_1, 1), \dots,
  (a_{\ell-1}, b_{\ell-1},0), (a_{\ell}, b_{\ell},1)\}.
\]
Define $Z_0 := \{(a_0, b_0,0)\}$, $U_0 := \{(a_0, b_0,1)\}$, and for $k>0$,
\[
  Z_k := Z_{k-1} \cup \{(a_k, b_k,0)\}, \quad 
  U_k := U_{k-1} \cup \{(a_k, b_k,1)\}.
\]
The first stage of our procedure computes the Cayley table of a \ldto for 
$Z_0 \cup U_\ell$. In the second stage, we do the same for
$Z_1 \cup U_\ell$, and so on, until we arrive at 
a Cayley table for $Z_\ell \cup U_\ell = A^2 \times \{0,1\}$.
So there are $\ell+1$ stages, each stage consisting of $\ell$ steps (described below), 
and each steps requiring a single call to Algorithm~\ref{alg:ld-2}.
Therefore, we need to make $\ell^2 \approx n^4$ calls to Algorithm~\ref{alg:ld-2}, so 
the total running-time is on the order of $r^2m^2n^7 + n^9$.

Here are the steps involved in the first stage, where we compute a \ldto for 
$Z_0 \cup U_\ell$. 
\begin{enumerate}[1.]
  \item Compute $p_1 \in \dtr S_1 ( = \dtr \{(a_0, b_0, 0), (a_0, b_0, 1)\})$.    
  \item compute $q_1\in \dtr \{(a_0, b_0, 0), (p_1(a_1, a_1, b_1), b_1, 1)\}$; \\
  form $p_2(x,y,z) = q_1(p_1(x,y,z), p_1(y,y,z), z)$; % \in \dtr S_2$; %\{(a_0, b_0, 1), (a_1, b_1, 1),(a_{\ell}, b_{\ell},0)\}$.
  \item compute $q_2\in \dtr \{(a_0, b_0, 0), (p_2(a_2, a_2, b_2), b_2, 1)\}$; \\
    form $p_3(x,y,z) = q_2(p_2(x,y,z), p_2(y,y,z), z)$;\\ % \in \dtr S_3$;\\
  $\vdots$
  \item[$\ell$.] compute $q_{\ell-1}\in \dtr \{(a_0, b_0, 0),  (p_{\ell-1}(a_{\ell-1}, a_{\ell-1}, b_{\ell-1}), b_{\ell-1}, 1)\}$; \\
  form
  $p_\ell(x,y,z) = q_{\ell-1}(p_{\ell-1}(x,y,z), p_{\ell-1}(y,y,z), z)$;
\end{enumerate}
Call the final result $s_0 := p_\ell$.
We prove below that $s_0 \in \dtr (Z_0 \cup U_\ell)$.

The second stage is very similar to the first; however, when running through the $\ell$ steps,
as in the list above, our goal will be to arrive at $s'_1 \in 
\dtr \bigl((a_1, s_0(a_1, b_1, b_1), 0) \cup U_\ell\bigr)$. After generating such an operation 
we define $s_1(x,y,z) := s'_1(x, s_0(x,y,y), s_0(x,y,z))$, which will
belong to $\dtr \bigl(Z_1 \cup U_\ell\bigr)$ (as we prove below).
We continue in this way for $(\ell+1)$ stages of $\ell$ steps each, until we arrive at 
our goal: $d \in \dtr \bigl(Z_\ell \cup U_\ell\bigr)$.

Without further ado, here is a precise description of the algorithm.
\RestyleAlgo{boxruled}
%\LinesNumbered
\begin{algorithm}
  % \KwIn{$\tabT \eta_{\kk}$ and $\tabT \sA_{2k}$}
  \KwOut{$p_\ell \in \dtr (Z_0 \cup U_\ell)$}
...
\begin{algorithm}  
  \KwIn{$s_{k-1} \in \dtr (Z_{k-1} \cup U_\ell)$}
  \KwOut{$p_\ell \in \dtr \bigl\{(a_k, s_{k-1}(a_k, b_k, b_k), 0) \cup U_\ell\bigr\}$}
...
\begin{algorithm}  
\KwOut{$s_\ell \in \dtr (Z_\ell \cup U_\ell)$}
...
\begin{lemma}
  The operation $s_0$ returned by Algorithm~\ref{alg:new-2-1} is a \ldto for $S_\ell$.
  $Z_0 \cup U_\ell$
\end{lemma}
...
\begin{proof}
  \wjd{TODO: fill in these proofs!}
\end{proof}


\wjd{TODO: summarize and discuss complexity.}
...
\newpage
  ...
  \emph{Proof.}
  %\sA_2 &:=\{(a_\ell, b_\ell, 1), (a_0, b_0, 0), (a_0, b_0, 1)\},\\
  % $\sA_0 =$
  % $p_1 \in \dtr \{(a_0, b_0, 0), (s(a_{\ell}, a_{\ell}, b_{\ell}), b_{\ell}, 1)\}$);
  % $q_1\in \dtr \{(a_1, p_1(a_1, b_1, b_1), 0), (s(a_{\ell}, a_{\ell}, b_{\ell}), b_{\ell}, 1)\}$; \\
  % form $p_2(x,y,z) = q_1(x, p_1(x,y,y), s(x,y,z))$;
  \begin{alignat*}{3}
  ...
  Define $S_0 := \{(a_{\ell}, b_{\ell},0)\}$ and let
  $S_{k+1} = S_{k} \cup \{(a_k, b_k, 1)\}$; in particular,
  $S_\ell := \{(a_0, b_0, 1), (a_1, b_1, 1), \dots,(a_{\ell-1}, b_{\ell-1},1), (a_{\ell}, b_{\ell},0)\}$.
  % \begin{align*} 
  %   S_1 &:= S_0 \cup \{(a_0, b_0, 1) \},\\
  % S_2 &:= S_0 \cup \{(a_0, b_0, 1), (a_1, b_1, 1)\},\\
  % & \vdots\\
  % S_k &:= S_0 \cup  \bigcup_{0\leq j < k}\{(a_j, b_j, 1)\}\\
  % & \vdots\\
  ...
  \begin{enumerate}[1.]
    ...
    \item compute $q_2\in \dtr \{(p_2(a_2, a_2, b_2), b_2, 1), (a_{\ell}, b_{\ell},0)\}$; \\
      form $p_3(x,y,z) = q_2(p_2(x,y,z), p_2(y,y,z), z)$;\\ % \in \dtr S_3$;\\
      % \item compute $q_3\in \dtr \{(p_3(a_3, a_3, b_3), b_3, 1), (a_{\ell}, b_{\ell},0)\}$; \\
    % form $p_4(x,y,z) = q_3(p_3(x,y,z), p_3(y,y,z), z) \in \dtr S_4$;\\
    $\vdots$
  ...
  \begin{algorithm}
    % \KwIn{$\tabT \eta_{\kk}$ and $\tabT \sA_{2k}$}
    \KwOut{$p_\ell \in \dtr S_{\ell}$}
    
    compute $p_1 \in \dtr S_1$. % for $\{(a_0, b_0, 1), (a_{\ell}, b_{\ell},0)\}$.    
...
%\LinesNumbered
\begin{algorithm}
  \KwIn{$s \in \dtr S_\ell$}
  \KwOut{$p_\ell \in \dtr\bigl( A^2 \times \{0,1\}\bigr)$}
  
  compute $p_1 \in \dtr \{(a_0, b_0, 0), (s(a_{\ell}, a_{\ell}, b_{\ell}), b_{\ell}, 1)\}$. % for $\{(a_0, b_0, 1), (a_{\ell}, b_{\ell},0)\}$.    
...
\end{algorithm}
  ...
  $\gamma_i := \Cg^{\alg{A}}(a_{k+1}, c_i)$ and $\delta_i := \com{\gamma_i}$. % (in $O(rm^2 + n^5)$ time).
  ...
  \]
  \wjd{FIX THIS SECTION! We don't want to generate the sets $\tabT \zeta_{\kk}$; they're too big!}
