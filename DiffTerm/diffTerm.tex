...
\begin{proof}
  ...
  The analysis follows that of Section~\ref{sec:algorithm-its-time} 
  (especially the proof of Proposition~\ref{speedprop}).  Recall, $n = |A|$, and 
  $m=\|\alg A\| = \sum_{i=0}^r k_i n^i$, where $k_i$ is the number of basic 
  operations of arity~$i$, and $r$ is the largest arity of the basic 
  operations of $\alg{A}$. The following complexity bounds are easily derivable 
  from the proof of Proposition~\ref{speedprop}: $\thetaab$,
...
The following proposition provides another avenue for constructing a polynomial-time algorithm to decide if a finite idempotent algebra generates a variety that has a difference term (see Theorem~\ref{thm:time}).
\begin{prop}
...
Let $\alg{A} = \<A, \dots\>$ be a finite idempotent algebra.
 and suppose we know that a difference term operation for $\alg{A}$ exists.  
...
is the value $t^{\alg{A}}(a_1, \dots, a_k)$, and whose 
$(x_1, \dots, x_k)$-entry is the value $t^{\alg{A}}(x_1, \dots, x_k) = u$.  
...
the table of the corresponding term operation.
That is, we want to know the value $t^{\alg{A}}(a_1, \dots, a_k)$ for all inputs from $A^k$.
For this purpose, we will use the notation $\Cayley_{x_1,\dots, x_k \mid u}$ to 
refer to such an operation table. 
...
\begin{algorithm}%[ht]
  ...
  \Return $L$.
  \caption{\ldto tables for $\{(a_0,b_0,0), (a_1, b_1, 1)\}$}
  \label{alg:ld-2}
...
We postpone the implementation details and computational complexity analysis of 
...
%$\sA_2 := \{(a_0, b_0, 0), (a_0, b_0, 1), (a_1,b_1,0), (a_1,b_1,1)\}$, $\dots$,
For $k > 0$, $\sA_{2k} := \sA_{2k-1} \cup \{(a_k,b_k,1)\}$  and $\sA_{2k+1} := \sA_{2k} \cup \{(a_{k+1},b_{k+1},0)\}$.
...
The latter will be input to Algorithm~\ref{alg:stream-ldt2}, the result of which is
...
the Cayley table of a \ldto for $\sA_{2n} = A^2 \times \{0,1\}$,
since that will represent a difference term operation for $\alg A$ (as noted
...
\begin{algorithm}
  \SetKwInOut{Input}{Input}
  \SetKwInOut{Output}{Output}
  \caption{\ldto table for $\sA_{2k+1}$ \label{alg:stream-ldt1} {\small ($k\geq 0$)}}
  \Input{$\eta_{\kk}$ and $s_{2k} \in \dtr \sA_{2k}$}
  \Output{$\eta_{\kplus}$ and $s_{2k+1} \in \dtr \sA_{2k+1}$}

  construct $p \in \eta_{\kk} \cap \dtr \{ (a_{k+1}, s_{2k}(a_{k+1}, b_{k+1}, b_{k+1}), 0)\}$

  construct $s_{2k+1}(x,y,z) = p(x, s_{2k}(x,y,y), s_{2k}(x,y,z))$;

  construct $\eta_{\kplus} = \eta_{\kk}
              % \cap \dtr \{(a_k, b_k, 1)\}$;
              \cap \eta_k$;

  return $(s_{2k+1}, \eta_{\kplus})$.
\end{algorithm}

\subsection{Implementation of Algorithm~\ref{alg:stream-ldt1}}
We are given a list $L_k$ of Cayley tables of all operations in
$\eta_{\kk} = \dtr \{(a_0, b_0, 1), \dots, (a_{k-1}, b_{k-1}, 1) \}$.
We want to find the Cayley tables of all operations in  the set
% $\eta_{\kk} \cap \dtr \{\}$.
\[
\dtr \{(a_0, b_0, 1), \dots, (a_{k-1}, b_{k-1}, 1), (a_{k+1}, s_{2k}(a_{k+1}, b_{k+1}, b_{k+1}), 0) \}.
\]
Let $a := a_{k+1}$ and $b := s_{2k}(a_{k+1}, b_{k+1}, b_{k+1})$ in the definition: 
\[
t \in \dtr \{(a,b,0)\} \quad \iff \quad a\comr{\thetaab} t^{\alg{A}}(a,b,b).
\]
Compute $\thetaab := \Cg^{\alg{A}} (a_{k+1}, s_{2k}(a_{k+1}, b_{k+1}, b_{k+1}) )$
and $\delta_{ab} := \com{\thetaab}$, and then filter the list $L_k$ for those tables
whose $(a, b, b)$-entry belongs to $a/\delta_{ab}$.
...
\begin{algorithm}
  \SetKwInOut{Input}{Input}
  \SetKwInOut{Output}{Output}
  \caption{\ldto table for $\sA_{2k}$ \label{alg:stream-ldt2} {\small ($k> 0$)}}
  \Input{$\zeta_{\kminus}$ and $s_{2k-1} \in \dtr \sA_{2k-1}$}
  \Output{$\zeta_{\kk}$ and $s_{2k} \in \dtr \sA_{2k}$}

  construct $p \in \zeta_{\kminus} \cap
          \dtr \{(s_{2k-1}(a_k, a_k, b_k), b_k, 1) \}$;

  construct $s_{2k}(x,y,z) = p(s_{2k-1}(x,y,z), s_{2k-1}(y,y,z),z)$;

  construct $\zeta_{\kk} = \zeta_{\kminus}
            % \cap \dtr \{(a_{k-1}, b_{k-1}, 0)\}$;
            \cap \zeta_{k-1}$

  return $(s_{2k}, \zeta_{\kk})$.
\end{algorithm}
