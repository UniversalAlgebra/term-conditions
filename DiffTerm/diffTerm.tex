...
$\bbV(\alg{A})$ has a Taylor term and no type-\atyp\ tails
(equivalently, $\bbV(\alg{A})$ has no 1's and no type-\atyp\ tails).
...
Now, restricting to just testing no type-\atyp\ tails (vs no tails of any type) is
...
\section{The Algoritm and its Time Complexity}
...
Based on the results of the last  section we can construct
an algorithm to test if an idempotent $\alg A$ has a difference term that
runs in time a constant times $rn^6m^3$.
\textcolor{red}{Should we give a sketch of this algorithm?}
In this section we show that there is such an algorithm that
