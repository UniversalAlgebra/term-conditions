...
Here are the steps that Stage 1 takes in order to compute a \ldto for
...
\begin{algorithm}
  ...
  \ForAll {$1\leq i < {n^2}$} {
    ...
    Form the table $\sanst_{i+1}$, defined as follows:\\
    $\quad \sanst_{i+1}[x,y,z] = \sanss_i\bigl[\sanst_i[x,y,z], \sanst_i[y,y,z], z\bigr]$ $(\forall x,y,z)$;
  }
...
\KwOut{$\sansd_{n^2}$, the Cayley table of a \ldt op for $A^2 \times \{0,1\}$}
  ...
  Use Alg.~\ref{alg:new-2-1} to compute the table $\sansd_0$ of a \ldt op for 
  $Z_0 \cup U_{n^2}$\; %(Alg.~\ref{alg:new-2-1});
  ...
  \ForAll {$1\leq k <{n^2}$} {
    ...
    Form the table $\sansd_{k+1}$, defined as follows:\\
...
\end{document}
  ...
  \begin{comment}
    ...
    \begin{algorithm}
      ...
      \KwOut{$s'_{k} \in \dtr \bigl\{(a_k, s_{k-1}(a_k, b_k, b_k), 0) \cup U_{\ell}\bigr\}$}
    ...
    \begin{lemma}
      ...
      %Since $q_{\l}\in \dtr (a_k, c_k, 0)$,  %implies
    ...
    proving Claim 2.
    %notice that $s_1 \in \dtr \bigl(Z_1 \cup U_\ell)$.  Indeed,
