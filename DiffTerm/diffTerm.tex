...
proceed directly to the inductive stages of our method,
...
which is the table of a difference term operation for $\alg A$ (as noted
in Corollary~\ref{cor:loc-diff-term}).

\subsubsection{Induction Stage 1}
For a set $\sS$ of term operations, let $\sC \sS$ denote the set of 
...
\begin{algorithm}
  ...
  For each $\Cayley_j \in \sC \dtr \sA_{2k}$, form 
  \[
  \sG_j := \sC \eta_{\kk}  \cap \sC \dtr \{ (a_{k+1}, \Cayley_j[a_{k+1}, b_{k+1}, b_{k+1}], 0)\}.
  \]

  For each $\sansG_{j_i} \in \sG_j$, define the table $\Cayley_{j_i}$ as follows:
  $\Cayley_{j_i}[x,y,z] :=  \sansG_{j_i}[x, \Cayley_j[x,y,y], \Cayley_j[x,y,z]]$. \\[5pt]
  Let $\sC \sA_{2k+1} := \{\Cayley_{j_i}: i, j\}$ be the set of all tables so defined.\\[5pt]
  Build the set $\sC \eta_{\kplus} := \{\Cayley \in \eta_{\kk} \colon \Cayley[a_k,a_k,b_k]= b_k \}$.\\[5pt]
  Return $\sC \eta_{\kplus}$ and $\sC \sA_{2k+1}$.
\end{algorithm}

\subsubsection{Implementation of Algorithm~\ref{alg:stream-ldt1}}
Given the sets $\sC \eta_{\kk}$
and $\sC \dtr \sA_{2k}$, of all tables of operations in 
$\eta_{\kk}$ and $\dtr \sA_{2k}$, respectively,
Algorithm~\ref{alg:stream-ldt1} returns the sets
$\sC \eta_{\kplus}$ and $\sC \dtr \sA_{2k+1}$.
Building the set $\sC \eta_{\kplus}$ is trivial.  We simply remove 
from $\sC \eta_{\kk}$ every table whose $(a_k,a_k,b_k)$-entry is not $b_k$.
As for $\sC \dtr \sA_{2k+1}$,
for each table $\Cayley_j \in \sC \dtr \sA_{2k}$,
let 
$c_j:= \Cayley_j[a_{k+1}, b_{k+1}, b_{k+1}]$ and compute
$\gamma := \Cg^{\alg{A}}(a_{k+1}, c_j)$ and $\delta := \com{\gamma}$. % (in $O(rm^2 + n^5)$ time).
Next construct the tables in the set
$\eta_{\kk}\cap \dtr \{ (a_{k+1}, c_j, 0)\}$, according to the usual
definition: 
$t \in \dtr \{(a_{k+1},c_j,0)\}$ iff $a_{k+1} \comr{\gamma} t^{\alg{A}}(a_{k+1},c_j,c_j)$.
Thus 
$\sG_j = \{\Cayley \in \sC \eta_{\kk} \colon  a_{k+1} \mathrel{\delta} \Cayley [a_{k+1}, c_j, c_j]\}$.
For each $\sansG_{j_i} \in \sG_j$, define the operation table $\Cayley_{j_i}$ as follows:
for all $x$, $y$, $z \in A$, $\Cayley_{j_i}[x,y,z] :=  \sansG_{j_i}[x, \Cayley_j[x,y,y], \Cayley_j[x,y,z]]$. 
The set of all tables so defined is $\sC \sA_{2k+1}$.
...
\begin{algorithm}
  ...
  \Input{$\sC \zeta_{\kminus}$ and $\sC \dtr \sA_{2k-1}$}
  \Output{$\cC \zeta_{\kk}$ and $\sC \dtr \sA_{2k}$}

  For each $\Cayley_j \in \sC \dtr \sA_{2k-1}$, form 
  \[
  \sG_j := \sC \eta_{\kminus}  \cap \sC \dtr \{ (\Cayley_j[a_k, a_k, b_k], b_k, 1), 1)\}.
  \]

  For each $\sansG_{j_i} \in \sG_j$, define the table $\Cayley_{j_i}$ as follows:
  $\Cayley_{j_i}[x,y,z] :=  \sansG_{j_i}[\Cayley_j[x,y,z], \Cayley_j[y,y,z], z]$. \\[5pt]
  Let $\sC \sA_{2k} := \{\Cayley_{j_i}: i, j\}$ be the set of all tables so defined.\\[5pt]
  Build the set $\sC \zeta_{\kk} := \{\Cayley \in \zeta_{\kminus} \colon a_{k-1} \mathrel{\delta_{k-1}} \Cayley[a_{k-1},b_{k-1},b_{k-1}] \}$.\\[5pt]
  Return $\sC \zeta_{\kk}$ and $\sC \sA_{2k}$.
\end{algorithm}

\subsubsection{Implementation of Algorithm~\ref{alg:stream-ldt2}}
Given the sets $\sC \zeta_{\kminus}$
and $\sC \dtr \sA_{2k-1}$, of all tables of operations in 
$\zeta_{\kminus}$ and $\dtr \sA_{2k-1}$, respectively,
Algorithm~\ref{alg:stream-ldt2} returns the sets
$\sC \zeta_{\kk}$ and $\sC \sA_{2k}$.
To build $\sC \dtr \sA_{2k}$, take each table $\Cayley_j$ from $\sC \dtr \sA_{2k-1}$
and set $c_j:= \Cayley_j[a_k, a_k, b_k]$. Then, let $\sG_j$ denote 
the set of tables in $\sC \zeta_{\kminus}$ whose 
$(c_j, c_j, b_k)$-entry is equal to $b_k$. 
For each $\sansG_{j_i} \in \sG_j$, define the operation table 
$\Cayley_{j_i}$ as follows:
for all $x$, $y$, $z \in A$, 
$\Cayley_{j_i}[x,y,z] :=  \sansG_{j_i}[\Cayley_j[x,y,z], \Cayley_j[y,y,z], z]$. 
The set of all tables so defined is $\sC \sA_{2k}$.
Finally, to build the set $\sC \zeta_{\kk}$, we 
compute $\gamma_{k-1} := \Cg^{\alg{A}}(a_{k-1}, b_{k-1})$ and $\delta := \com{\gamma_{k-1}}$,
and then remove from $\sC \zeta_{\kminus}$ every table whose 
