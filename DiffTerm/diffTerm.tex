...
``\emph{Assume $\Gamma$}'' will mean ``Assume $\sS$, $\alg{B}$, $n$, $\{\alg{A}_i \mid i < n\}$,
...
of $\Clo_3(\alg{A})$.  If we permit ourselves to overload notation
for the moment, then we could take
\begin{align*}
...
\begin{align*}
  ...
  \text{ let }\\
  \dtr S &= \{t \in \Clo_3(\alg{A}) \mid (s,t) \in \dtr \text{ for all } s \in S\}, \text{ and }\\
  \breve{\dtr} T &= \{s \in A^2 \times \{0,1\} \mid (s,t) \in \dtr \text{ for all } t \in T\}.
\end{align*}
...
In this section we describe an algorithm for finding the operation table 
of a difference term operation for a finite idempotent algebra.
...
We present the algorithm in stages. 
Algorithm~\ref{alg:ld-2} gives a procedure for finding a \ldto for sets
...
To begin, note that the first and third projections, $t(x,y,z) = x$ and  $t(x,y,z) = z$, are
\ldtos for the sets $\{(a_0,b_0,1), (a_1, b_1, 1)\}$ and $\{(a_0,b_0,0), (a_1, b_1, 0)\}$,
respectively.
\begin{comment}
\footnote{Moreover, if $a_0 = b_0$, then by idempotence every ternary term $t$
satisfies $t(a_0, b_0, b_0) = a_0$; therefore, the third projection is a \ld
...
so the third projection is a \ldto.
...
a \ldto for such sets can be computed by
Algorithm~\ref{alg:ld-2}, described in the box below.

\newcommand{\Cayley}{\ensuremath{\mathsf{C}}}
...
% and whose $(x_1, \dots, x_k)$-entry is the value $t^{\alg{A}}(x_1, \dots, x_k) = u$.
If $\sansC$ is such a table, we will denote the 
$(a_1, \dots, a_k)$-entry of the table by $\sansC[a_1, \dots, a_k]$.
% Below we will encounter a situation in which we don't want to construct the syntactic
% term that produced the element $u$ from generators, but we do want to construct 
% the Cayley table of the term operation.
% That is, we want to know the value $t^{\alg{A}}(a_1, \dots, a_k)$ for every tuple from $A^k$. 
Alternatively, if we wish to emphasize the means by which 
we arrived at the term that the table represents, we may use
% $\Cayley_{x_1,\dots, x_k \mid u}[a_1, \dots, a_k]$, or simply
$\Cayley_{x_1,\dots, x_k \mid u}$ to denote the table.
% (This is simply a notational device, not a well-defined mathematical object.)
...
\begin{algorithm}%[ht]
  ...
  \KwIn{$S = \{(a_0,b_0,0), (a_1, b_1, 1)\}$}
  \KwOut{$(n\times n)^3$-array (table) for an operation in $\dtr S$}

  compute $\delta_0 = \com{ \theta_0 }$ and form $C_0= (a_0/\delta_0) \times \{b_1\}$\;
  compute $S_0=\Sg^{\alg{A}\times \alg{A}} ((a_0,a_1),(b_0,a_1),(b_0,b_1))$\;
  \ForAll{$(u,v)  \in S_0$} {
    compute the table $\Cayley_{(a_0,a_1),(b_0,a_1),(b_0,b_1)\mid (u,v)}$\;
    \If{$(u,v) \in C_0 \cap S_0$} {
    add $\Cayley_{(a_0,a_1),(b_0,a_1),(b_0,b_1) \mid (u,v)}$ to list $L$\;
    }
  }
  \Return $L$.
  \caption{Generate the list of all Cayley tables of \ldtos for $\{(a_0,b_0,0), (a_1, b_1, 1)\}$}
  \label{alg:ld-2}
  % in the $((a_0,a_1),(b_0,a_1),(b_0,b_1))$ position

\end{algorithm}
Each table returned by Algorithm~\ref{alg:ld-2} represents a ternary operation of type
$(A\times A)^3 \to A\times A$, whose value at $((a_0,a_1),(b_0,a_1),(b_0,b_1))$ is 
an element of $C_0 \cap S_0$.

We postpone the implementation details and computational complexity analysis
of Algorithm~\ref{alg:ld-2} (see Section~\ref{sec:cc-ld-2}) and 
proceed directly to the inductive stage of our method,
for which Algorithm~\ref{alg:ld-2} serves as a base case. 
...
%$\sA_2 := \{(a_0, b_0, 0), (a_0, b_0, 1), (a_1,b_1,0), (a_1,b_1,1)\}$, $\dots$,
and for all $0< k < n^2-1$, $\sA_{2k} := \sA_{2k-1} \cup \{(a_k,b_k,1)\}$, and $\sA_{2k+1} := \sA_{2k} \cup \{(a_{k+1},b_{k+1},0)\}$.

The output of Algorithm~\ref{alg:ld-2} is a list of Cayley tables of
\ldtos for $\sA_0$, and this list is input to Algorithm~\ref{alg:stream-ldt1},
the output of which is a list of Cayley tables of \ldtos for $\sA_1$.
The latter is input to Algorithm~\ref{alg:stream-ldt2}, the result of which is
a list of Cayley tables of \ldtos for $\sA_2$.
Thereafter, the process alternates between
Algorithms~\ref{alg:stream-ldt1} and~\ref{alg:stream-ldt2}.
This procedure terminates when we find 
the Cayley table of a \ldto for $\sA_{2n^2-1} = A^2 \times \{0,1\}$,
since that will be the table of a difference term operation for $\alg A$ (as noted
...
\end{align*}
For $\sS$ a set of term operations, let $\sC \sS$ denote the set of 
all Cayley tables of operations in $\sS$. 
...
\begin{algorithm}
  ...
  \Input{$\sC \eta_{\kk}$ and $\sC \dtr \sA_{2k}$}
  \Output{$\cC \eta_{\kplus}$ and $\sC \dtr \sA_{2k+1}$}

  Build the set $\sG := \sC \eta_{\kk}  \cap \sC \dtr \{ (a_{k+1}, s_{2k}(a_{k+1}, b_{k+1}, b_{k+1}), 0)\}$.

  For each $\sansG \in \sG$, define $\Cayley[x,y,z] = \sansG[x, s_{2k}(x,y,y), s_{2k}(x,y,z)]$; let 
  $\sC \sA_{2k+1}$ be the set of tables so defined.

  Build the set $\sC \eta_{\kplus} := \{\Cayley \in \eta_{\kplus} \mid \Cayley[a_k,a_k,b_k]= b_k \}$

  Return $\sC \eta_{\kplus}$ and $\sC \sA_{2k+1}$.
\end{algorithm}

\subsection{Implementation of Algorithm~\ref{alg:stream-ldt1}}
Given the set $\sC \eta_{\kk}$ of tables of all operations in
$\eta_{\kk} = \dtr \{(a_0, b_0, 1), \dots, (a_{k-1}, b_{k-1}, 1) \}$, and
the set $\sC \dtr \sA_{2k}$ of tables of all operations in $\dtr \sA_{2k}$,
Algorithm~\ref{alg:stream-ldt1} should return the sets
$\sC \eta_{\kplus}$ and $\sC \dtr \sA_{2k+1}$.
To start we find tables of all operations in the set
$\eta_{\kk}\cap \dtr \{ (a_{k+1}, s_{2k}(a_{k+1}, b_{k+1}, b_{k+1}), 0)\}$.
% \[\dtr \{(a_0, b_0, 1), \dots, (a_{k-1}, b_{k-1}, 1), (a_{k+1}, s_{2k}(a_{k+1}, b_{k+1}, b_{k+1}), 0) \}.\]
Let $\alpha := a_{k+1}$ and $\sigma := s_{2k}(a_{k+1}, b_{k+1}, b_{k+1})$ in the definition: 
\[
t \in \dtr \{(\alpha,\sigma,0)\} \quad \iff \quad \alpha \comr{\theta_{\alpha \sigma}} t^{\alg{A}}(\alpha, \sigma, \sigma).
\]
Compute $\theta_{\alpha \sigma} := \Cg^{\alg{A}} (a_{k+1}, s_{2k}(a_{k+1}, b_{k+1}, b_{k+1}))$
and $\delta_{\alpha\sigma} := \com{\theta_{\alpha\sigma}}$. 
% Let $\mathbf{a} := (a_0, a_1, \dots, a_k, \alpha)$, $\mathbf{b} := (b_0, b_1, \dots, b_k, \sigma)$, $\mathbf{b'}:= (b_0, b_1, \dots, b_k, \sigma)$.
% and compute $S_k = \Sg^{\alg{A}^{k+2}}\{\mathbf a, \mathbf b, \mathbf b'\}$.
Then $\sC \dtr \sA_{2k+1} = \{\Cayley \in \sC \eta_{\kk} \mid  \alpha \mathrel{\delta_{\alpha \sigma}} \Cayley [\alpha, \sigma, \sigma] \}$.
...
\begin{algorithm}
  \SetKwInOut{Input}{Input}
  \SetKwInOut{Output}{Output}
  \caption{Generate the set of Cayley tables of all \ldtos for $\sA_{2k}$ \label{alg:stream-ldt2} {\small ($k> 0$)}}
  \Input{$\sC \zeta_{\kminus}$ and $\sC \dtr \sA_{2\kminus}$}
  \Output{$\cC \zeta_{\kk}$ and $\sC \dtr \sA_{2k}$}

  Build the set $\sG := \sC \zeta_{\kminus}  \cap \sC \dtr \{(s_{2k-1}(a_k, a_k, b_k), b_k, 1) \}$.

  For each $\sansG \in \sG$, let $\Cayley[x,y,z] = \sansG[s_{2k-1}(x,y,z), s_{2k-1}(y,y,z),z]$;
  let $\sC \sA_{2k}$ be the set of tables so defined.

  Build the set $\sC \zeta_{\kk} := \{\Cayley \in \zeta_{\kminus} \mid a_{k-1} \mathrel{\delta_{k-1}} \Cayley[a_{k-1},b_{k-1},b_{k-1}]\}$.

  Return $\sC \zeta_{\kk}$ and $\sC \sA_{2k}$.
\end{algorithm}

\subsection{Computational Complexity of Algorithm~\ref{alg:ld-2}}
\label{sec:cc-ld-2}
...
\begin{enumerate}
\item compute $\thetao$, in time $crm$;
...
Algorithm~\ref{alg:ld-2} is at most $crm + c(rm^2 + n^5) + c r^2m^2n^3$.
...
\begin{comment}
(An equivalent approach, that may be easier to implement in software is
to partially produce the subalgebra of $\alg{A}^{\alg{A}^3}$ generated by
the three $|A|^3$-tuples
$g_0 = (a_0, a_1, \dots)$,
$g_1 = (b_0, a_1, \dots)$,
$g_2 = (b_0, b_1, \dots)$,
each of which is extended so that every 
triple from $A^3$ is represented as $(g_0(i), g_1(i), g_2(i))$ for some 
unique $i$.  Producing the entire subuniverse will be expensive in 
general, but we only need to pay attention to the first two coordinates. 
At each step, we check for a tuple $u = (u_0, u_1, u_2, \dots)$ such that
$(u_0, u_1)\in C_0 \cap S_0$.  If we find one, then the entire tuple $u$ stores
the table for a \ldto for $((a_0, b_0, 0), (a_1, b_1, 1))$ and we can stop.
...
\begin{enumerate}
  ...
  \item Let $L_{2k} := \{\mathsf{C} \in L_{2k-1} \mid \mathsf{C}[a_k, b_k, b_k] \mathrel{\delta_{k}} a_k \}$.
