...
A few more bits of notation will come in handy below.
Given a set $S \subseteq A^2\times \{0,1\}$,
and a set $T \subseteq \Clo_3(\alg{A})$ of ternary term operations 
on $\alg{A}$, we denote by $\bowtie$ the relation from $T$ to $S$ defined 
as follows: $t \bowtie (a,b,i)$ iff the following conditions hold:
%% \begin{align*}
...
%% $(\Fix, \Inv)$---namely,
\begin{align*}
\Inv &\colon \sP(A^2 \times \{0,1\}) \to \sP(\Clo_3(\alg{A})) \text{ and }\\
\Fix &\colon \sP(\Clo_3(\alg{A})) \to \sP(A^2\times \{0,1\})
\end{align*}
