...
\]
% \end{equation*}
The last equality holds since $q \in \Inv{S_1}$.
...
\begin{proof}
  ...
  % $d \in \Inv{(a,b,0)}$,
  $d$ is \ldto for $(a,b,0)$, and $d(a,a,b) = b$, since
  % $d\in \Inv{(a,b,1)}$.
  $d$ is \ldto for $(a,b,1)$.
\end{proof}
%
...
\begin{proof}
  ...
  and that 
  $t \in \Inv{\{(a,b,0), (a',b',1)\}}$ if and only if
  \[
...
that the ordering of the first three steps is inconsequential.\footnote{
  ...
  \begin{enumerate}
    ...
    as a stream $S_0$; filter $S_0$ against the predicate
...
\end{align*}
% Evidently, $\zeta_{\kk} = \bigcap_{0\leq i < k}\zeta_i$ is the set of \ldtos
% for $\{(a_i, b_i, 0) \mid 0\leq i<k\}$, while
% $\eta_{\kk} = \bigcap_{0\leq i < k}\eta_i$ is the set of \ldtos
