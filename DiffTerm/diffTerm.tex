...
\subsection{Algorithm to test for existence of a difference term operation}
\label{sec:algor-1}
...
\begin{proof}
  By the previous proposition, we only need to determine if each 3-generated subalgebra of 
  $\alg A^2$ has a difference term operation. This could be accomplished by applying the 
  algorithm from Corollary~\ref{cor:algor-1} at most $\binom{n^2}{3}$ ($\approx n^6$) times, 
  for a total running time of $O(rm^2n^{10} + n^{13})$ (cf.~$O(rn^4m^4 + n^{14}),$ 
  the difference term existence complexity bound from Theorem~\ref{thm:time}).
\end{proof}
