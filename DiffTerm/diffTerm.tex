...
$(a_1, \dots, a_k)$-entry of the table by $\sanst[a_1, \dots, a_k]$.
...
\begin{algorithm}%[ht]
  ...
  \ForAll{$(u,v)  \in S_0$} {
    compute the table $\sanst_{(a_0,a_1),(b_0,a_1),(b_0,b_1)\mid (u,v)}$\;
    \If{$(u,v) \in C_0 \cap S_0$} {
    include $\sanst_{(a_0,a_1),(b_0,a_1),(b_0,b_1) \mid (u,v)}$ in the set $\sT$\;
    }
  }
  \Return $\sT$.
...
\end{algorithm}
Each table in the set $\sT$ returned by Algorithm~\ref{alg:ld-2} represents a 
ternary operation of type $(A\times A)^3 \to A\times A$, whose value at 
$((a_0,a_1),(b_0,a_1),(b_0,b_1))$ is an element of $C_0 \cap S_0$.
...
For a set $\sS$ of operations, let $\sT\sS$ denote the set of 
all Cayley tables representing operations in $\sS$. 

The output of Algorithm~\ref{alg:ld-2} is the set $\sT \sA_0$ of all 
Cayley tables of operations in $\sA_0$, the \ldtos for $\{(a_0, b_0, 0), (a_0, b_0, 1)\}$.
The set $\sT \sA_0$ is input to Algorithm~\ref{alg:stream-ldt1},
the output of which is the set $\sT \sA_1$ of all tables of 
operations in $\sA_1$.
The latter is input to Algorithm~\ref{alg:stream-ldt2}, the result of which is
the set $\sT \sA_2$ of tables for $\sA_2$.
...
%\LinesNumbered
\begin{algorithm}
  \KwIn{$\sT \eta_{\kk}$ and $\sT \sA_{2k}$}

  \KwOut{$\sT \eta_{\kplus}$ and $\sT \sA_{2k+1}$}

  \ForAll{$\sanst_j \in \sT \sA_{2k}$} {

    $\sT_j := \sT \eta_{\kk} \cap \sT \dtr (a_{k+1}, \sanst_j[a_{k+1}, b_{k+1}, b_{k+1}], 0)$;

    \ForAll{$\sanss_i \in \sT_j$} {
      \ForAll {$x$, $y$, $z \in A$}  {
        %define the table $\sanss_{j_i}$ as follows:
        $\sansr_{ji}[x,y,z] :=  \sanss_i[x, \sanst_j[x,y,y], \sanst_j[x,y,z]]$;
      }
      include $\sansr_{ji}$ in the set $\sT$;
    }
  }
%  Let $\sT[\sA_{2k+1}] := \{\sanss_{ji}: i, j\}$ be the set of all tables so defined.\\[5pt]
  compute $\sS  := \bigl\{\sanst \in \sT \eta_{\kk} \colon \sanst[a_k,a_k,b_k]= b_k \bigr\}$;
...
Given the sets $\sT \eta_{\kk}$
and $\sT \sA_{2k}$, of all tables of operations in 
$\eta_{\kk}$ and $\sA_{2k}$, respectively,
Algorithm~\ref{alg:stream-ldt1} returns the sets
$\sT \eta_{\kplus}$ and $\sT \sA_{2k+1}$.
Building the set $\sT \eta_{\kplus}$ is trivial.  We simply remove 
from $\sT \eta_{\kk}$ every table whose $(a_k,a_k,b_k)$-entry is not $b_k$.
As for $\sT \sA_{2k+1}$,
for each table $\sanst_j \in \sT \sA_{2k}$,
let 
$c_j:= \sanst_j[a_{k+1}, b_{k+1}, b_{k+1}]$, compute
...
Thus 
$\sT_j = \{\sanst \in \sT \eta_{\kk} \colon  a_{k+1} \mathrel{\delta} \sanst [a_{k+1}, c_j, c_j]\}$.
For each $\sanss_{i} \in \sT_j$, define the operation table $\sansr_{ji}$ as follows:
for all $x$, $y$, $z \in A$, $\sansr_{ji}[x,y,z] :=  \sanss_{i}[x, \sanst_j[x,y,y], \sanst_j[x,y,z]]$. 
The set of all tables $\{\sansr_{ji} \colon i, j\}$ so defined is $\sT \sA_{2k+1}$.
...
%\LinesNumbered
\begin{algorithm}
  \KwIn{$\sT \zeta_{\kminus}$ and $\sT \sA_{2k-1}$}
  \KwOut{$\sT \zeta_{\kk}$ and $\sT \sA_{2k}$}

  \ForAll{$\sanst_j \in \sT \sA_{2k-1}$} {
  
    $\sT_j := \sT \eta_{\kminus}  \cap \sT \dtr (\sanst_j[a_k, a_k, b_k], b_k, 1)$;

    \ForAll{$\sanss_{i} \in \sT_j$} {
      \ForAll {$x$, $y$, $z \in A$}  {
        $\sansr_{ji}[x,y,z] :=  \sanss_{i}[\sanst_j[x,y,z], \sanst_j[y,y,z], z]$;
      }
      include $\sansr_{ji}$ in the set $\sT$;
    }
  }

  compute $\sS := \{\sanst \in \sT \zeta_{\kminus} \colon a_{k-1} \mathrel{\delta_{k-1}} \sanst[a_{k-1},b_{k-1},b_{k-1}]\}$;
...
Given $\sT \zeta_{\kminus}$
and $\sT \sA_{2k-1}$, the sets of all tables in
$\zeta_{\kminus}$ and $\sA_{2k-1}$, respectively,
Algorithm~\ref{alg:stream-ldt2} returns the sets
$\sT \zeta_{\kk}$ and $\sT \sA_{2k}$.
To build $\sT \sA_{2k}$, take each table $\sanst_j$ from $\sT \sA_{2k-1}$
and set $c_j:= \sanst_j[a_k, a_k, b_k]$. Then, let $\sT_j$ denote 
the set of tables in $\sT \zeta_{\kminus}$ whose 
$(c_j, c_j, b_k)$-entry is equal to $b_k$. 
For each $\sanss_{i} \in \sT_j$, define the operation table 
$\sansr_{ji}$ as follows:
for all $x$, $y$, $z \in A$, 
$\sansr_{ji}[x,y,z] :=  \sanss_{j_i}[\sanst_j[x,y,z], \sanst_j[y,y,z], z]$. 
The set of all tables so defined is $\sT \sA_{2k}$.
Finally, to build the set $\sT \zeta_{\kk}$, we 
compute $\gamma_{k-1} := \Cg^{\alg{A}}(a_{k-1}, b_{k-1})$ and $\delta := \com{\gamma_{k-1}}$,
and then remove from $\sT \zeta_{\kminus}$ every table whose 
