...
\textcolor{purple}{Below is an older version, which will
need to be deleted. Also the time estimate is wrong.}
...
$i \in \{0,1\}$.
%% An \defin{$\alg A $-local difference term for
A \defin{local difference term operation for $(a,b,i)$} 
is a ternary term operation $d$ satisfying the following:
\begin{align}
%% \text{ if $i=0$, then } & a \comm{\Cg^{\alg A }(a,b)}{\Cg^{\alg A }(a,b)} d(a,b,b); \label{eq:diff-triple}\\
...
\begin{proof}
  ...
  and this is an \ld term operation for $\alg A $.  Indeed, if $d$ is a
...
\begin{proof}
  ...
  then the first projection is an \ld term. If $i = i' = 1$,
    ...
    (Observe that $a/\delta \times \{b'\}$ is a subalgebra of $\alg A  \times \alg A $
    by idempotence.)
...
\begin{algorithm}%[ht]
  ...
  \caption{return an \ld term for $\{(a_0,b_0,0), (a_1, b_1, 1)\}$
          \label{alg:ld-2}  }
\Input{$\{(a_0,b_0,0), (a_1, b_1, 1)\}$}
\Output{an \ld term for $\{(a_0,b_0,0), (a_1, b_1, 1)\}$}
...
\textcolor{red}{(Should we say explicitly how to perform Step 4 of Algorithm~\ref{alg:ld-2}?)}
...
\begin{equation}
\label{eqn:notation1}
\theta_i = \Cg^{\alg{A}}(a_i, b_i) \; \text{ and } \;
\delta_i=\com{\theta_i},
\end{equation}
we also take
$as = ((a_0, b_0), (a_1, b_1), \dots, (a_k, b_k))$ 
...
compute the stream $\sS_1$ of term operations $s$ that satisfy $s(a_0,a_0,b_0) = b_0$;

find $p$ in $\sS_1$ satisfying $p(a_0, b_0, b_0) \mathrel{\delta_0} a_0$;

compute $c_1 = p(a_1,b_1,b_1)$;

find $q$ in $\sS_1$ satisfying $q(a_1, c_1, c_1) \mathrel{\delta_1} a_1$;

return $(\sS_1, s_1)$ where $s_1(x,y,z) = q(x,p(x,y,y),p(x,y,z))$
\end{algorithm}
...
\begin{algorithm}
  ...
  \Input{$(\sS_1, s_1)$ (from Alg~\ref{alg:stream-ldt1})}
  \Output{an \ld term for $\sA_2$}

compute the stream $\sS_2$ of term operations $p$ 
that satisfy $p(s_1(a_1,a_1,b_1), s_1(a_1,a_1,b_1), b_1) = b_1$;

find $p$ in $\sS_1 \cap \sS_2$ and compute $d_1 = p(a_1,b_1,b_1)$;

find $q$ in $\sS_2$ satisfying $q(a_1, d_1, d_1) \mathrel{\delta_1} a_1$.

return $(\sS_2, s_2)$ where $s_2(x,y,z) = q(x, p(x,y,y), p(x,y,z))$
\end{algorithm}
