...
The next lemma follows from the same argument used to prove~\cite[Lemma~3.1]{Freese:2009}, using the fact that since $\beta$ is abelian over $\alpha$ it follows that $\gamma$ over $\delta$ that appears in the proof will also have type 2.
...
We note that since $\alg A$ is simple of type~\btyp then the ternary projection operation $p(x,y,z ) = z$ is a difference term operation for $\alg A$.  We also note that the term operation $(x\cdot y) \cdot(y \cdot x)$ of $\alg A$ is commutative and so the variety generated by $\alg A$ omits type~\utyp.  So, this example also demonstrates that for finite idempotent algebras, having a difference term operation and generating a variety that omits type~\utyp does not guarantee the existence of a difference term for the variety.
\end{example}
...
block, we join the block containing $x$ and the one containing~$y$
...
$\delta_{i+1}$ form $\delta_i$ is $c(n^4 + rm)$. Since the congruence
...
$\alg A$ has a Taylor term (give an example), it is true when
...
using methods that are entirely different than the ones used in the previous
sections.
...
$A \times \{0,1\}$, and $|S|$ will denote the length of
the list. The definition above of a \ldt for
...
with terms in $\Clo_3(\alg{A})$ that are \ldt for
$(a, b, i)$.  That is,
\[
...
\end{align*}
In other words, $\dtr(S)$ is the set of \ld terms
...
every subset of $\AsqBool$ of size $k \leq n$ has an \ld term. Let
\[
S := \{(a_0, b_0, \chi_0), (a_1, b_1, \chi_1), \dots,
        (a_{n}, b_{n},\chi_{n})\} \subseteq \AsqBool,
\]
so $|S| = n+1$.  We prove $S$ has an \ld term (i.e., $\dtr(S)$ is nonempty).
...
then $|S_0| < |S|$, so $S_0$ has a \ldt term, $q \in \dtr(S_0)$.
We now show that
\[
...
S_1 := \{(a_0, b_0, 0), (a_1, b_1 0), \dots, (a_{k-1}, b_{k-1}, 0),
        (p(a_n, a_n, b_n), b_n, 1)\},
\end{equation*}
then $|S_1| < |S|$, so there exists $q \in \Inv{S_1}$.
We claim  that
\begin{equation*}
...
% $q \in \Inv{S_1}\subseteq \Inv{p(a_n, a_n, b_n), b_n, 1)}$.
$q$ is \ldt for $(p(a_n, a_n, b_n), b_n, 1)$.
...
\begin{proof}
  ...
  the existence of a term operation $d \in \Inv{S}$.
  Thus, for all $a, b \in A$, we have that $a \comr{\thetaab} d(a,b,b)$, since
  % $d \in \Inv{(a,b,0)}$,
  $d$ is \ldt for $(a,b,0)$, and $d(a,a,b) = b$, since
  % $d\in \Inv{(a,b,1)}$.
  $d$ is \ldt for $(a,b,1)$.
\end{proof}
%
...
Algorithm~\ref{alg:ld-2} serves as a base case, giving an \ld term for
$\sA_0$ that we will use as input to Algorithm~\ref{alg:stream-ldt1}, the output of which is an \ld term for $\sA_1$.
...
\begin{algorithm}%[ht]
  ...
  \caption{Return \ldt for $\sA_{2k+1}$, given
  $\eta_{\kk}$ and \ldt for $\sA_{2k}$.
  \label{alg:stream-ldt1}  }
  \Input{$\eta_{\kk}$ and $s_{2k} \in \Inv { \sA_{2k} }$}
  \Output{$\eta_{\kplus}$ and $s_{2k+1} \in \Inv { \sA_{2k+1} }$}

  construct $p \in \eta_{\kk} \cap \Inv {\{ (a_{k+1}, s_{2k}(a_{k+1}, b_{k+1}, b_{k+1}), 0)\} }$
...
\begin{algorithm}%[ht]
  ...
  \Input{$\zeta_{\kminus}$ and $s_{2k-1} \in \Inv{ \sA_{2k-1} }$}
  \Output{$\zeta_{\kk}$ and $s_{2k} \in \Inv {\sA_{2k}}$}

  construct $p \in \zeta_{\kminus} \cap
          \Inv { \{(s_{2k-1}(a_k, a_k, b_k), b_k, 1) \} }$;

  construct $s_{2k}(x,y,z) = p(s_{2k-1}(x,y,z), s_{2k-1}(y,y,z),z)$;

  construct $\zeta_{\kk} = \zeta_{\kminus}
            % \cap \Inv { \{(a_{k-1}, b_{k-1}, 0)\}}$;
            \cap \zeta_{k-1}$
