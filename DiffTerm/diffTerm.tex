...
\begin{align}
\text{ if $i=0$, then } & a \comr{\thetaab} t(a,b,b); \label{eq:diff-triple}\\
\text{ if $i=1$, then } & t(a,a,b) = b. \nonumber
\end{align}
If $t$ satisfies~(\ref{eq:diff-triple}) for all triples
in some subset $S\subseteq A^2 \times \{0,1\}$, then we call $t$
...
the list. The definition above of a difference term for 
a set has an obvious analog for a list: if $t$ satisfies~(\ref{eq:diff-triple}) for all triples $(a,b,i)$ appearing in 
the list $S$, %\in \bigl(A^2 \times \{0,1\}\bigr)^n$, 
then $t$ is called a \defin{local difference term operation for $S$}.

% For example,
% if $S = ((a_0, b_0, \chi_0), \dots, (a_n, b_n, \chi_n))$, then
% $|S| = n+1$.  

A few more notational conventions will come in handy below.
Given a set (or list) $S$ whose elements belong to $A^2\times \{0,1\}$,
and a set (or list) $T$ whose elements belong to
$\Clo_3(\alg{A})$ (the set of ternary term operations of $\alg A$),
we denote by $\ltimes$ the relation from $T$ to $S$ defined 
by $t \ltimes (a,b,i)$ iff the relations~(\ref{eq:diff-triple})
are satisfied.
%% \begin{align*}
...
% \end{cases}
% \end{align*}
Occasionally, $(a,b,i)\rtimes t$ will serve as a convenient and equivalent alternative to $t\ltimes (a,b,i)$.

As a binary relation, $\ltimes$ induces an obvious Galois connection
from subsets of $A^2\times \{0,1\}$ to subsets
of $\Clo_3(\alg{A})$.  Specifically, we define
%% $(\Fix, \Inv)$---namely,
\begin{align*}
^{\rtimes} &\colon \sP(A^2 \times \{0,1\}) \to \sP(\Clo_3(\alg{A})) \text{ and }\\
^{\ltimes} &\colon \sP(\Clo_3(\alg{A})) \to \sP(A^2\times \{0,1\})
\end{align*}
as follows:
for $S \subseteq A^2 \times \{0,1\}$ and $T \subseteq \Clo_3(\alg{A})$,
\begin{align*}
  S^{\rtimes} &= \{t \in \Clo_3(\alg{A}) \mid t \ltimes s
  \text{ for all } s \in S\}, \\
...
\]
so that $|S| = n+1$.  We prove $S$ has an \ld term. That is, $S^{\rtimes} \neq \emptyset$.
...
\]
Since $|S'| < |S|$, there exists $p \in S'^{\rtimes}$.
...
then $|S_0| < |S|$, so there exists $q \in S_0^{\rtimes}$.
We now show that 
\[
...
\begin{corollary}
  ...
  Then $\Inv {\bigl(\AsqBool\bigr)} \neq \emptyset$,
  and so $\alg{A}$ has a difference term operation.
\end{corollary}
\begin{proof}
  Letting $S := \AsqBool$ in Theorem~\ref{thm:local-diff-terms} establishes
  the existence of a term operation $d \in \Inv{S}$. Thus, for all $a, b \in A$, $a \comr{\thetaab} d(a,b,b)$, since 
  % $d \in \Inv{(a,b,0)}$, 
  $d \ltimes (a,b,0)$, and $d(a,a,b) = b$, since 
  % $d\in \Inv{(a,b,1)}$.
  $d\ltimes (a,b,1)$.
\end{proof}
% 
...
\begin{proof}
  ...
  called into question is the commutator, but there is a straight-forward polynomial-time algorithm for computing it (see, e.g.,~\cite{2017arXiv170302764D}).
\end{proof}
