...
it generates. Then $\sV$ has a difference term if and only if
\begin{enumerate}
\item $\alg A$ has a Taylor term.
\item There do not exist $a$, $b$, $c$, and $d\in A$
such that, if $\alg B$ is
the subalgebra of 
$\alg A \times \alg A$ generated by $0 = (a, c)$, $1 = (b,c)$ 
and $t = (a,d)$, and $\beta = \Cga01{B}$ is join irreducible
with lower cover $\alpha$, and $\beta$ over $\alpha$ has type~\atyp,
and $\rho_0 \join \alpha = 1_{\alg B}$.
\end{enumerate}
\end{theorem}
