...
Algorithm~\ref{alg:ld-2}, as described in the box below.
...
\begin{algorithm}%[ht]
  ...
  \caption{\ldto table for $\{(a_0,b_0,0), (a_1, b_1, 1)\}$\label{alg:ld-2}}
  \Input{$S = \{(a_0,b_0,0), (a_1, b_1, 1)\}$}
  \Output{$(n\times n)^3$-array (table) for an operation in $\dtr S$}

  compute $\delta_0 = \com{ \theta_0 }$ and form $C_0= (a_0/\delta_0) \times \{b_1\}$;

  compute $S_0=\Sg^{\alg{A}\times \alg{A}} ((a_0,a_1),(b_0,a_1),(b_0,b_1))$;

  find an element $(u,v)  \in C_0 \cap S_0$;

  return the table for the operation that produced $(u,v)$. 
  % in the $((a_0,a_1),(b_0,a_1),(b_0,b_1))$ position

\end{algorithm}
The table returned by Algorithm~\ref{alg:ld-2} represents a ternary operation
$t : (A\times A)^3 \to A\times A$, and the value at 
position $((a_0,a_1),(b_0,a_1),(b_0,b_1))$ of the table is an element $(u,v)$ belonging 
to $C_0 \cap S_0$.
...
The output of Algorithm~\ref{alg:ld-2} is the table of a 
\ldto for $\sA_0$ which will be input to Algorithm~\ref{alg:stream-ldt1},
the output of which is the table of a \ldto operation for $\sA_1$.
The output of Algorithm~\ref{alg:stream-ldt1} is input to
Algorithm~\ref{alg:stream-ldt2}, the result of which is
the table of a \ldto for $\sA_2$.
...
the table of a \ldto operation for $\sA_{2n} = A^2 \times \{0,1\}$,
since that will be the 
table of a difference term operation for $\alg A$ (as we saw in 
Corollary~\ref{cor:loc-diff-term} above).
...
in time $c(rm^2 + n^5)$;
\item generate $S_0$, %a subuniverse of $\alg{A}\times \alg{A}$,
  in time $c r m^2$;
  \begin{enumerate}
  \item for each new element of $S_0$, compute the full
    table of the operation that produced it;
  \item if $(u,v) \in C_0$, then return the operation table from (3a);
    else, cache the table and continue generating $S_0$.
  \end{enumerate}
\end{enumerate}
...
from $S_0$, and the operation tables producing these pairs were
already stored (at Step (3a)).  Thus, to compute the table
...
We only need to run the algorithm once since, as soon as a \ldto is found for
a single (arbitrary) triple of the form $((a,a'),(b,a'),(b,b'))$,
