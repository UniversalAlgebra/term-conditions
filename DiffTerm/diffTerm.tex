...
\begin{proof}
  ...
  called into question is the commutator, but, as we saw in the proof of
  Proposition~\ref{speedprop} above, there is a straight-forward
  polynomial-time algorithm for computing it.
  %% or, for more details,
  %% see~\cite{2017arXiv170302764D}).
\end{proof}
...
\begin{proof}
  By the previous proposition, we only need to determine if each 3-generated subalgebra of $\alg A^2$ has a difference term operation.  So, we only need to apply the algorithm from Corollary~\ref{cor:algor-1} at most $\binom{n^2}{3}$ times, where $n = |A|$.
\end{proof}
...
\begin{algorithm}%[ht]
  ...
  \caption{Return the table of a \ldto for $\{(a_0,b_0,0), (a_1, b_1, 1)\}$ \label{alg:ld-2}  }
  \Input{$S = \{(a_0,b_0,0), (a_1, b_1, 1)\}$}
  \Output{operation table for $t \in \dtr S$}
...
return the operation table for $t$
\end{algorithm}
\begin{comment}
...
Therefore,
\end{comment}
We now discuss the implementation details and computational cost of
Algorithm~\ref{alg:ld-2}, using the notation introduced in
Section~\ref{sec:algorithm-its-time}.  Recall,
$n = |A|$, and $m=\|\alg A\| = \sum_{i=0}^r k_i n^i$,
where $k_i$ is the number of basic operations of arity~$i$, and $r$
is the largest arity of the basic operations of $\alg{A}$.
The complexity bounds in this section are easily derivable 
from Proposition~\ref{speedprop}.

Algorithm~\ref{alg:ld-2} could be implemented as follows:
\begin{enumerate}
...
and we conclude that no \ldto exists for this pair.  The runtime of this
procedure is bounded by $|A|^2$.)
...
We are now ready to describe the inductive stages of our algorithm,
for which the following notation is useful:
%% \begin{align*}
...
Algorithm~\ref{alg:ld-2} serves as a base case, giving a \ldto for
$\sA_0$ that we will use as input to Algorithm~\ref{alg:stream-ldt1},
the output of which is a \ldto for $\sA_1$.
That output will serve in turn as input to
Algorithm~\ref{alg:stream-ldt2} the result of which is
a \ldto for $\sA_2$.
Thereafter, this process alternates between
Algorithms~\ref{alg:stream-ldt1} and~\ref{alg:stream-ldt2}.
Inductively, we obtain a single \ldto for all of $A^2 \times \{0,1\}$,
...
\begin{algorithm}
  \SetKwInOut{Input}{Input}
  \SetKwInOut{Output}{Output}
  \caption{Return \ldto for $\sA_{2k+1}$, given
  $\eta_{\kk}$ and \ldto for $\sA_{2k}$.
...
\begin{algorithm}
  \SetKwInOut{Input}{Input}
  \SetKwInOut{Output}{Output}
  \caption{Return a \ldto for $\sA_{2k}$ given $\zeta_{\kk}$ and
  a \ldto for $\sA_{2k-1}$.
