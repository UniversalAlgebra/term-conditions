...
\LinesNumbered
\begin{algorithm}
  \KwIn{$\sC \eta_{\kk}$ and $\sC \dtr \sA_{2k}$}
  \KwOut{$\cC \eta_{\kplus}$ and $\sC \dtr \sA_{2k+1}$}

  \ForAll{$\Cayley_j \in \sC \dtr \sA_{2k}$} {
    compute the tables
    \[
    \sG_j := \sC \eta_{\kk}  \cap \sC \dtr \{ (a_{k+1}, \Cayley_j[a_{k+1}, b_{k+1}, b_{k+1}], 0)\}.
    \]
    \ForAll{$\sansG_{j_i} \in \sG_j$} {
      define the table $\Cayley_{j_i}$ as follows:
      $\Cayley_{j_i}[x,y,z] :=  \sansG_{j_i}[x, \Cayley_j[x,y,y], \Cayley_j[x,y,z]]$.
    }
  }
  Let $\sC \dtr \sA_{2k+1} := \{\Cayley_{j_i}: i, j\}$ be the set of all tables so defined.\\[5pt]
  Build the set $\sC \eta_{\kplus} := \{\Cayley \in \eta_{\kk} \colon \Cayley[a_k,a_k,b_k]= b_k \}$.\\[5pt]
  \Return $\sC \eta_{\kplus}$ and $\sC \dtr \sA_{2k+1}$.
...
\LinesNumbered
\begin{algorithm}
  \KwIn{$\sC \zeta_{\kminus}$ and $\sC \dtr \sA_{2k-1}$}
  \KwOut{$\cC \zeta_{\kk}$ and $\sC \dtr \sA_{2k}$}

  \ForAll{$\Cayley_j \in \sC \dtr \sA_{2k-1}$} {
    compute the tables
    \[
...
Given $\sC \zeta_{\kminus}$
and $\sC \dtr \sA_{2k-1}$, the sets of all tables in
