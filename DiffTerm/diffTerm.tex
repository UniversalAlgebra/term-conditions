...
gives a procedure (Algorithm~\ref{alg:ld-2}) for finding a \ldto for sets
of size 2, and Section~\ref{sec:induct} gives two inductive steps
(Algorithms~\ref{alg:stream-ldt1} and~\ref{alg:stream-ldt2})
for producing \ldtos on successively larger subsets of $A^2 \times \{0,1\}$.
...
\begin{algorithm}%[ht]
  ...
  \caption{Return a \ldto for $((a_0,b_0,0), (a_1, b_1, 1))$ \label{alg:ld-2}  }
...
returning a term $t$, or at least the operation table for $t$.

Therefore, we implement Algrithm~\ref{alg:ld-2} as follows: first compute
$\delta_0 = \com{ \thetao }$ and form $C_0= (a_0/\delta_0) \times \{b_1\}$;
then generate $S_0$ and, at every stage,
(temporarily) store the term tree for each newly generated element of $S_0$;
if an element of $S_0$ belongs to $C_0$, then the term tree that produced the
element is a \ldto for $S_0$; in that case, compute and return the operation
table of this term (without necessarily computing all of $S_0$).
If a newly generated element of $S_0$ does not belong to $C_0$, then
(discard the term tree and) continue generating $S_0$.
...
For $k > 0$, 
%&\vdots\\
$\sA_{2k} := \sA_{2k-1} \cup \{(a_k,b_k,1)\}$,
%\{(a_0, b_0, 0), (a_0, b_0, 1), \dots, (a_k,b_k,0), (a_k,b_k,1)\}.
...
$\sA_0$ that we will use as input to Algorithm~\ref{alg:stream-ldt1}, the output of which is a \ldto for $\sA_1$.
That output will serve in turn as input to
Algorithm~\ref{alg:stream-ldt2} the result of which is
a \ldto for $\sA_2$.  Thereafter, this process alternates between Algorithms~\ref{alg:stream-ldt1} and~\ref{alg:stream-ldt2}.
