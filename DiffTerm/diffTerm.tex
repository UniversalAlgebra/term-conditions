...
Computers have become an invaluable tool in the quest to understanding 
...
\wjd{We should insert an example of an
  algebra $\alg A$ that has a difference term operation, but
  $\bbV(\alg A)$ has no difference term.}
...
has an \ld term.  
% That is, suppose $\Inv(s_0, s_1)$ is nonempty for every 
% pair $(s_0, s_1) \in (\AsqBool)^2$. 
