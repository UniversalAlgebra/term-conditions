...
\subsection{Algorithm to test for existence of difference term operation}
\label{sec:algor-1}
...
give an algorithm (Subroutine LD-2) for finding an \ld term for sets 
of size 2, and in Subsection~\ref{sec:induct} we give two inductive steps 
(Subroutines~\ild-0 and~\ild-1) on which our recursive algorithm will depend.
The full recursive algorithm for constructing a difference term 
operation for $\alg A$ is given in Subsection~\ref{sec:algor-2}.
...
\Output{\ld term for $\{(a_0,b_0,0), (a_1, b_1, 1)\}$}
...
\textcolor{red}{We should say explicitly how to perform Step 4 of Algorithm~\ref{alg:ld-2}.}
...
\begin{remarks}\
\begin{enumerate}[1.] 
\item In each of Algorithms~\ref{alg:ild0} and \ref{alg:ild1},
...
\textcolor{red}{(Decide whether to omit the proof entirely.)}
...
% \end{equation}
be a listing of all pairs in the set $A\times A$.
