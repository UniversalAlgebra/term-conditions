...
we denote by $\bowtie$ the relation from $T$ to $S$ defined as follows:
...
As a binary relation, $\bowtie$ induces an obvious Galois connection
...
for $S$, and $\Fix(T)$ is the set of triples for which every $t\in T$ 
is an \ld term.  Sometimes $S$ will denote a finite list,
say $S = ((a_0, b_0, \chi_0), \dots, (a_n, b_n, \chi_n))$, in which case the set of \ld term operations for $S$ will be denoted by 
$\Inv((a_0, b_0, \chi_0), \dots, (a_n, b_n, \chi_n))$.
...
If $S' := S \mysetminus \{(a_n, b_n, \chi_n)\}$, then
$|S'| < |S|$, so the set $S'$ has an \ld term, $p$.
...
If $\gamma := \Cg(a_n, p(a_n,b_n,b_n))$, then
\begin{equation}
    \label{eq:100000}
  d(a_n,b_n,b_n) =
...
S_0 := \{(a_0, b_0, 0), (a_1, b_1 0), \dots, (a_{k-1}, b_{k-1}, 0), 
        (p(a_n, a_n, b_n), b_n, 1)\},
\end{equation*}
then $|S_0| < |S|$, so there exists $q \in \Inv(S_0)$. We claim  that
