...
%%%%%%%%%%%%%%%%%%%%%%%%%%%% wjd: NEW SECTION  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Difference Term Operation for One Algebra}
Above we addressed the problem of deciding the existence 
of a difference term for a given (idempotent, locally finite) 
variety.  In this section we are concerned with the existence of a 
difference term operation for a specific (finite, idempotent) algebra, 
and we will describe polynomial-time algorithms for
\begin{enumerate}
...
\end{enumerate}
Notice that Theorem XX already solves Problem~(\ref{item:a}).
...
(and the proof doesn't use any tame congruence theory).  
Second, this alternative proof develops machinery 
that we need to solve Problem~(\ref{item:b}).
...
\noindent {\bf Notation:} In the sequel we will often use the abbreviation
``\ld term'' in place of ``local difference term.''

\smallskip

Let
$\sS = A \times A \times \{0,1\}$. We usually denote an 
...
\noindent {\bf Notation:} \emph{Throughtout the proof of Theorem~\ref{thm:local-diff-terms}, we let}
\begin{equation*}
\theta_i := \Cg(a_i,b_i).
\end{equation*}
...
\begin{proof}
  ...
  obviously an \ld term for the whole set
  $A\times A \times \{0,1\}$.
...
Throughout this section, for elements $a_i$ and $b_i$ of an algebra $\alg{A}$, 
we denote by $\theta_i$ the congruence generated by $(a_i,b_i)$.  That is,
\begin{equation*}
...
the situation handled in Subroutine 01.  Nonetheless, we include the
listing here so that later we can easy refer to this version of the algorithm.
\begin{enumerate}
...
There are only two other sets of size three to consider, 
since the other cases are symmetric to these.
They are
\begin{equation*}
\{(a_0, b_0, 0), (a_1, b_1, 1),  (a_2, b_2, 0)\} \text{ and }
...
% and the pair $(t(a_2,a_2,a_2), t(a_2,b_2,b_2))$
% clearly belongs to $\Cg(a_2,b_2)$.
% Therefore, $\Cg(a_2, t(a_2,b_2,b_2)) \leq \Cg(a_2,b_2)$.
