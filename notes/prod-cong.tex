...
\end{proof}
  ...
  % so
  % $(d^{\bF}(x,x,y), y) \in \ker \phi$. Therefore, we must have
...
So, % $\com{\Cg^{\bF}(x,y)} = \Cg^{\bF}(x,y)$. 
assume
%% $\com{\Cg^{\bF}(x,y)} < \Cg^{\bF}(x,y)$.
$[1_F, 1_F]< 1_F$.
In particular, $(x,y) \notin [1_F, 1_F]$.
...
%% interval from  $[1_F, 1_F]$ up to $1_F$ is abelian, thus the
...
$(y, x/[1_{F_1},1_{F_1}]) \in R/(\etaR_0 \meet \rho_1)$,
which implies $(y, z) \in R$ for some $z \comm{1_{F_1}}{1_{F_1}} x$.
That is, (\ref{eq:1}) holds.
We can rule out case~(\ref{item:8}) because $\bF_1/[1_{F_1}, 1_{F_1}]$ is abelian and
is therefore absorption free. (See~\cite{MR3374664}.)  %% and~\cite{Bergman-DeMeo}.)
So the minimal absorbing subuniverse of $\bF_1/[1_{F_1}, 1_{F_1}]$ is 
$F_1/[1_{F_1}, 1_{F_1}]$ itself.

We are left with case~(\ref{item:7}).
Let $B_0$ be a minimal absorbing subuniverse of $\bF_0$.
Then the product algebra $\bB_0\times \bF_1/[1_{F_1}, 1_{F_1}]$ is
absorbtion free.
Since $\bR/(\etaR_0 \meet \rho_1)$ is subdirect,
$\bR/(\etaR_0 \meet \rho_1) \cap \bB_0 \times F_1/[1_{F_1}, 1_{F_1}]$ is
non-empty. Therefore,
\begin{equation}
  \label{eq:2}
\bR/(\etaR_0 \meet \rho_1) \cap \bB_0 \times F_1/[1_{F_1}, 1_{F_1}]
= \bB_0 \times F_1/[1_{F_1}, 1_{F_1}].
\end{equation}
This follows from 
the Absorption Theorem (Theorem~\ref{thm:absorption} below)
...
$\bB_0 \times F_1/[1_{F_1}, 1_{F_1}] \subseteq \bR/(\etaR_0 \meet \rho_1)$,
so ${y} \times \bigl(x/[1_{F_1}, 1_{F_1}]\bigr) \in \bR$.
