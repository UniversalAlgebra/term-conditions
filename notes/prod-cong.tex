...
\subsection{Other observations that might be helpful}
...
Let's see if we can at least prove something useful in the following special
case: Suppose
\[
\bF:= \bF_{\bbV(\bA)}(x,y) \sdp \bA_0 \times \bA_1 \times \bA_2
\]
where $\bA_i = \Sg^{\bA}(a_i, b_i)$ for each $i\in \{0,1,2\}$.
Suppose also that $d$ is a
difference term operation when interpreted in $\bA_i$ for each 
$i\in \{0,1,2\}$, and $d$ is also a difference term operation when interpreted in
$\bA_i\times \bA_j$ for all $i \neq j$ in $\{0,1,2\}$.
Under these conditions, we will try to prove that $d$ is a difference term when
interpreted in $\bF$.

Let $\bx = (a_0,a_1,a_2)$ and $\by = (b_0,b_1,b_2)$ be the generators of $\bF$.
Then (I think) we can assume without loss of generality that $a_i$ and $b_i$ are
the generators of $\bA_i$ (TODO: check this!).
...
\end{equation}
Note that~(\ref{eq:4}) is stronger than the relation 
\begin{equation}
  \label{eq:5}
\bigl(d^{\bA_0}(a_0, b_0, b_0), d^{\bA_1}(a_1, b_1, b_1), d^{\bA_2}(a_2, b_2, b_2)\bigr)
...
Certainly, if (\ref{eq:5}) fails to hold, then so does (\ref{eq:4}),
equivalently (\ref{eq:3}). Hence
(\ref{eq:5}) is a necessary but not sufficient condition for (\ref{eq:3}).
\bigskip

By assumption, 
$d^{\bA_i}(a_i, b_i, b_i) \comr{1_{A_i}} a_i$,
for each $i \in \{0,1,2\}$.
