...
\label{sec:facts}
We start out by collecting in Lemma~\ref{lem:preliminary-facts} 
...
\smallskip
\item \label{item:2}
The algebra $\bF/\com{\Cg^{\bF}(x,y)}$ is abelian.
...
But first we require some more vocabulary.
...
and if $\Cg^{\bF}(x,y)$ is a neutral congruence, then
...
%% Recall $x/\Cg^{\bF}(x,y)$ denotes the congruence class of $\Cg^{\bF}(x,y)$
%% containing $x$. This class obviously contains both $x$ and $y$.
%% %% Now, consider $\com{\Cg^{\bF}(x,y)}$, the commutator of $\Cg^{\bF}(x,y)$ with itself.
Of course, $\com{\Cg^{\bF}(x,y)} \leq \Cg^{\bF}(x,y)$ and  
%% Equally clear, every idempotent term $t$ satisfies
%% \[t^{\bF}(y,x,x) \mathrel{\Cg^{\bF}(x,y)} t^{\bF}(y,y,y) = y.\]
Lemma~\ref{lem:cong-neut} handles the congruence neutral case in which these
congruences are equal. %% $\com{\Cg^{\bF}(x,y)} = \Cg^{\bF}(x,y)$. 
Assume
...
$[1_F, 1_F]$ up to $1_F$ is abelian, thus the algebra
