...
\begin{remark}
  ...
  Let $\pi_A$ and $\pi_B$ be the first and second projections of $\bR$ onto $\bF$.
  Since $\bF$ is idempotent, $\{x\}$ is a subalgebra of $\bF$, hence the inverse
  image of $\{x\}$ under $\pi_A$ is a subuniverse of $\bR$, call it $S_A$.
  Since $(x,y)$ and $(x,x)$ lie in $S_A$, and $\{x,y\}$
  generates $\bF$, we see that $\{x\}\times F$ is contained in $S_A$.
  In particular, for every $a_2$, the pair $(x,a_2)$ must lie in $R$.
  Similarly, there is a subalgebra $S_B$ containing
  $F\times \{x\}$. %% Note that the kernels of $\pi_A$ and $\pi_B$ are $\eta_A$ and $\eta_B$. 
  Finally, for any $(a_1,a_2)$ and $(b_1,b_2)$ in R we have
  \[
  (a_1,a_2) \mathrel{\eta_B} (x,a_2) \mathrel{\eta_A} (x,x)
  \mathrel{\eta_B} (b_1,x) \mathrel{\eta_A} (b_1,b_2).
  \]
