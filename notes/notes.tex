...
\title[Complexity of Difference Term Existence]{%
  Notes on Complexity of Deciding Existence of Difference Terms}
\date{\today}
%% \author{DeMeo}
...
\begin{lem}[{\protect cf.~\cite[Lem.~3.1]{Freese:2009}}]
  \label{lem:fv_3-1}
...
For $i \leq n$, let $\rho_i$ denote the kernel of the projection of $\bB$ onto $\bA_i$,
...
\begin{lem}[{\protect cf.~\cite[Lem.~3.2]{Freese:2009}}]
  \label{lem:fv_3-2}
  If $\sigma \subset \nn$, then
...
\begin{lem}[{\protect cf.~\cite[Lem.~3.3]{Freese:2009}}]
  \label{lem:fv_3-3}
  \begin{enumerate}[(i)]
...
\begin{proof}
  ...
  \item %\label{item:6} There exists $0\leq i< n $  such that $\alpha \join \rho_i = 1_B$
    If not, then we would have $\beta \leq \rho_i$ for all $i$, and that
  ...
  \item %\label{item:7} There exists $i$ such that $\alpha \join \rho_i < 1_B$.
    ...
    \noindent To prove the subclaim, we first note that $\beta \join \rho_\sigma = 1_B$ implies
    ...
    $a \mathrel{\rho_\sigma} t$.  Let $b\in \body(U)$ be such that
    $(a,b) \in \restr{\beta}{U} - \restr{\alpha}{U}$.  By Lemma~\ref{lem:fv_3-1},
    $\bB = \Sg^{\bB}(a, b, t)$.

    Let $v \in B$. Then $v = s^{\bB}(a,b,t)$ for some (idempotent) term $s$, so
    \[
    v = s^{B}(a,b,t) \mathrel{\rho_\sigma} s^{B}(a,b,a) \mathrel{\beta} s^{B}(a,a,a)  = a,
    \]
    and
    \[
    v = s^{B}(a,b,t) \mathrel{\beta}  s^{B}(a,a,t) \mathrel{\rho_\sigma}s^{B}(a,a,a)  = a.
    \]
    Therefore, $(v,a) \in  \beta \circ \rho_\sigma \cap \rho_\sigma \circ \beta$.
    ...
    \end{itemize}
    For all $a, b \in \body(U)$, the unary polynomials
    $d(x,a,b)$, $d(a,x,b)$, and $d(a,b,x)$ are permutations on $U$.

    For $c\in \body(U)$, let $p(x) = d(x,a,c)$. Then (see~\cite[Lem.~4.20]{HM:1988})
    we have $p(U) = U$, $p(a) = d(a,a,c) = c \in \body(U)$ and $p(t)\in \tail(U)$,
    since $t\in \tail(U)$.  Since $(a,t) \in \rho_\sigma$, we have
    $(c, p(t)) = (p(a), p(t)) \in \rho_\sigma$.

    We conclude from the foregoing that,
    for all nonempty proper subsets $\sigma \subset n$,
    for all $a\in \body(U)$ and all $v\in B$, we have
    $(a, v) \in \beta \circ \rho_\sigma \cap \rho_\sigma \circ \beta$, proving the subclaim.

    Now, \cite[Lem.~4.20]{HM:1988} states that it is impossible to find
    $b \in \body(U)$ and $t \in \tail(U)$ such that $d(b,t,t) \mathrel{\beta} b$.
    We will contradict this with the $b$ and $t$ considered above.
    In fact, for these elements, we even have equality: $d(b,t,t) = b$.
...
\input{inputs/global-local-diff-term.tex}
