...
The set of types is ordered according to the following ``lattice of types:''
...
\end{tikzpicture}
\end{center}
Then whether or not $\bbV(\bA)$ omits one of the order ideals of the lattice of types can be
...
In~\cite{Freese:2009}, the following corollary is the starting point of the
...
\begin{cor}[Cor.~2.2~\cite{Freese:2009}]
  ...
  In particular, $\bbV(\bA)$ omits 1 and 2 if and only if $S(\bA)$ omits 1 and 2.
\end{cor}
...
type-2 tails. Under the assumption that $n > 1$ we will attempt to prove that $n = 2$.
...
We begin with a list of perperties of the centralizer relation
