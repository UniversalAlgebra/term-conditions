...
\begin{abstract}
  This note is an attempt to understand the alternate description of the commutator 
  found in Kearnes~\cite[p.~930]{MR1358491}.
...
In~\cite[p.~930]{MR1358491} there seems to be a description of the
...
\begin{remark}
  ...
  $\downbeta \leq \Eq(A)$, it is guaranteed to have a least fixed point---namely, 
  $\nu := \bigcup_{k\geq 0} \Psiba^{k}(0_A)$.
  That is, $\Psiba(\nu) = \nu$
...
by~(\ref{item:1}).
    ...
    holds if and only if
    $\exists \, z_i \betar z_i'$ $(0\leq i \leq n)$,
    $\exists \, c_i \alphar c_i'$ $(0\leq i < n)$,
    $\exists\, f_i \in \Pol_1(\bA\times \bA)$ $(0\leq i < n)$
    such that
    $(x, x) = (z_0,z_0')$ and
    $(z_n,z_n') = (a,a)$, and
    ...
    \newcommand\gA{\ensuremath{g^{\bA}}}%
    %% \begin{align*}
    \[      f_i(c_i, c_i) = g_i^{\bbeta}((c_i, c_i), (a_1, a_1'), \dots, (a_k, a_k') )
      %% &= (g_i^{\bA}(x, a_1, a_2, \dots, a_k), g_i^{\bA}(x, a_1', a_2', \dots, a_k')),%
      = (g_i^{\bA}(c_i, \ba), g_i^{\bA}(c_i, \ba')),%
      %% \end{align*}%
      \]%
    \renewcommand\gA{\ensuremath{g}}%
    ...
    $\bigl((p(a, \bc), p(a, \bc')), (p(a', \bc), p(a', \bc'))\bigr)$
    belongs to  $\mathrel{\Delta_{\beta, \alpha}}$.
    Since  $(p(a, \bc), p(a, \bc')) \in \theta$, this shows
    $(p(a', \bc), p(a', \bc')) \in \Phiba(\theta)$ and  completes the proof.
  \end{proof}
