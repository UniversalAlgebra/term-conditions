...
\begin{abstract}
  The goal of this note is two-fold: first, we attempt to explain 
  the alternate description of the commutator given by Kearnes
  in~\cite[p.~930]{MR1358491}; second, we present another
  description of the commutator that we believe could form
  the basis of an easy and efficient (polynomial-time) algorithm for computing the commutator.
...
\item
  ...
  to tolerances. (See appendix below.)
  \item 
...
\item
  ...
  commutator, but the second ought to be especially easy to
  compute.  In other words, the following equation could be used as the basis for a
...
\begin{proof}\
  ...
  of $\Tol(A)$.
  Indeed, as we now prove, for a fixed $\theta \in \Tol(A)$, the following hold:
  \begin{enumerate}[(c.1)]
  ...
  \end{enumerate}
  To check~(c.\ref{item:c2}) note that $(a,b) \in \theta$
  implies $(a,b) \in \Psiba(\theta)$ because $(a,b)\mathrel{\Delta_{\beta, \alpha}} (a,b)$.
  ...
  $(a,b) \mathrel{\Delta_{\beta, \alpha}} (x,y)$; this and $(a,b) \in \gamma$ implies
  $(x,y) \in \Psiba(\gamma)$.
...
\item
  %% \noindent (ii) 
  We will show that $[\alpha, \beta]$ is the least fixed point of $\Psiba$ by
  verifying equation (\ref{eq:5}).
  ...
  $0_A \leq \Psiba(0_A) \leq \Psiba^{2}(0_A) \leq \cdots$.
  Thus, for each $n< \omega$,
  $\Join_{0\leq k\leq n} \Psiba^{k}(0_A) = \Psiba^{n}(0_A)$ and
  $\Join_{k\geq 0} \Psiba^{k}(0_A) = \lim_k \Psiba^{k}(0_A)$.
  Therefore, to prove~(\ref{eq:8}) it suffices to check that
  $\Psiba^{k}(0_A) \leq [\alpha, \beta]$ for all $k$.
