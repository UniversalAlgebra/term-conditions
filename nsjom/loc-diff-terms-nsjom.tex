...
\begin{proof}
% By definition,
% \[\phi \bigl(\Cg^{\bA}(a,a')\bigr) = \{\bigl(\phi (x),\phi(y)\bigr) \mid (x,y) \in \Cg^{\bA}(a,a') \}.\]
...
\section{Algorithm for Constructing a Difference Term Operation}
...
\alg{A} be the set $\{a_0, a_1, \dots, a_{n-1}, b_0, b_1, \dots, b_{n-1}\}$.
Let $T_0 = \{(a_0, b_0, 0), (a_1, b_1, 1)\}$,
$T_1 = \{(a_1, b_1, 1), (a_2, b_2, 0)\}$,
$T_2 = \{(a_2, b_2, 0), (a_3, b_3, 1)\}$, $\dots$,
$T_{n-1} = \{(a_{n-2}, b_{n-2}, 1), (a_{n-1}, b_{n-1}, 0)\}$.

First, we compute a local difference term for $T_0$ using the following
algorithm:

\noindent \underline{Subroutine 1:}
\begin{enumerate}
\item 
\end{enumerate}
