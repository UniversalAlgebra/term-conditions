...
\newcommand{\glocal}{glocal\xspace}
...
To wit, let $\sV$ be a variety and let $\bA_0=\< A_0, \dots\>$ and  $\bA_1=\< A_1, \dots\>$ be
algebras in $\sV$.  Fix $a_0, b_0 \in A_0$,  $a_1, b_1 \in A_i$,  and
$\chi_i \in \{0,1\}$.
%% An \defn{$\bA$-local difference term for
Then a ``mixed'' local difference term for the pair
$((a_0, b_0,\chi_0), (a_1, b_1,\chi_1))$
...
then $d$ is a mixed local difference term for $S$.
Elements of the disjoint union $\sU(A_0, A_1)$ will be denoted by
$\<(a, b, \chi), 0\>$ if $a, b \in A_0$ and
$\<(a, b, \chi), 1\>$ if $a, b \in A_1$.
We may use $\sU$ in place of $\sU(A_0, A_1)$ when the context renders the
...
Theorem~\ref{thm:local-diff-terms} above can be used to prove that every subset
$S\subseteq \sU$ has a mixed local difference term.
That is, there is a single term $d$ that works (i.e., satisfies
...
\begin{thm} %[\protect{cf.~\cite[Theorem 2.2]{MR3239624}}]
  ...
  has a local difference term. Then every tuple $S \subseteq \sU^n$
  has a local difference term.
\end{thm}
...
\begin{cor}
  ...
  $(\<(a_i,b_i,\chi_i),\iota_i\>,  \<(a_j,b_j,\chi_j),\iota_j\>)$ in  the square
  of the disjoint union, $\bigl(\coprod_{i \in I} (A_i^2\times \{0,1\})\bigr)^2$
...
and $i\in \{0,1\}$.
We call a term $d$ a \defn{global-local (or ``glocal'') difference term
for $(A, i)$}
...
% Recall, if $\scA$ is a collection of algebras belonging to a variety $\sV$,
% we let
% \[\scS(\scA) = \{(A, i) \mid \<A, \dots\> \in \scA \text{ and } i \in
% \{0,1\}\}.\]
\begin{cor}
...
\begin{proof}
  ...
  Our strategy is to prove that there is a term $d$ that 
  is a difference term for every subset of the product 
  ...
  operations required is to check whether $(\<(a, b, \chi), i\>, \<(a', b',
  \chi'), i'\>)$ has a local difference term is bounded by a
  polynomial in $|A_i||A_j|$.  Since there are 
  $4|A_i|^2|A_j|^2$ pairs in $\sU^2$, 
  it still takes only a polynomial in $|A_i||A_j|$ number of steps to test whether
  the pair $((A_i, \chi_i), (A_j, \chi_j))$ has a glocal difference term.
  There are $n^2 \leq |A|^4$ such pairs to test.  If
  $m = \max\{|A_i| \mid 0\leq i < n\}$, then the number of steps required to 
  test whether $\bF$ has a difference term operation is bounded by a constant times a
  power of $m|A|$.
\end{proof}
...
that
%% $(d(a, b,b), t(a,a,a))$ belongs to $\comm{\Cg(a,b)}{\Cg(a,b)}$.
