...
We call $t$ a \defn{global-local minority term} for 
...
we call $t$ a \defn{global-local minority term for $S$} if it is a
global-local minority term for every element $(A_i, \chi_i)$ in the sequence
$S$.

\begin{thm}
  Let $S \subseteq A\times A \times \{0,1,2\}$ and suppose that every
  triple $((a_0, b_0, \chi_0), (a_1, b_1, \chi_1), (a_1, b_1, \chi_1)) \in S^3$
  has a local minority term.  Then there is a term $t$ that is a local minority
  term for $S$.
\end{thm}
\begin{proof}
  ...
  $\sigma = ((a_0, b_0, \chi_0), (a_1, b_1, \chi_1), \dots, (a_{n}, b_{n}, \chi_{n}))$.
  Since $|\sigma|>3$, there exists $i \neq j$ such that $\chi_i = \chi_j$.
  Assume without loss of generality that $j=0$.
  Let $\sigma' = ((a_1, b_1, \chi_1), \dots, (a_{n}, b_{n}, \chi_{n}))$, which
  has a local minimal term by the induction hypothesis.

  Without loss of generality, suppose $\chi_0=0$, and $\chi_1 = \chi_2 =\cdots =\chi_k = 1$,
  and $\chi_{k+1} = \chi_{k+2} = \cdots = \chi_{n} = 1$.\\
  \\
  LEFT OFF HERE (TODO: complete this proof!)\\
  \\
