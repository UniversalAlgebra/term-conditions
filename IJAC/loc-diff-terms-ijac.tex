...
and $i\in \{0,1\}$.
We call a term $d$ a \defn{global-local difference term for $(A, i)$}
...
subuniverses of $\bA$. In the next theorem we will use some new notation and terminology.
Let $\sS(\bA):= \Sub(\bA) \times \{0,1\}$. For
\[S = ((A_0, \chi_0), (A_1, \chi_1), \dots,
(A_{n-1},\chi_{n-1})) \in \sS(\bA)^n,
\]
we call a term $d$ a \defn{global-local difference term for $S$}
if $d$ is a global-local difference term for every  $(A_i, \chi_i) \in S$.
Throughout this section we use $|S|$ to denote the \emph{length of the
sequence $S$}.
...
$S = ((A_0, \chi_0), (A_1, \chi_1), \dots, (A_{n},\chi_{n})) \in \sS(\bA)^{n+1}$.
%% so that $|S| = n+1$.  
...
$S' = ((A_1, \chi_1), \dots,(A_{n},\chi_{n}))$ of $S$. %% \in \sS(\bA)^{n}$
...
The first triple in $S$ is $(A_0, 0)$, so we need to show for all $a$, $b \in A_0$
that
\[
...
$t$ is a global-local difference term for $(A_0, 0)$, we have
\begin{equation}
  \label{eq:100100}
  d(a,b,b) 
...
that for all $a$, $b \in A_i$ we have
$d(a,a,b) = b$.
...
%% $(p(a,b,b), a)\in [\theta(a,b), \theta(a,b)]$.
This and (\ref{eq:451}) imply
that
%% $(d(a, b,b), t(a,a,a))$ belongs to $\comm{\theta(a,b)}{\theta(a,b)}$.
...
The first triple in $S$ is $(A_0, 1)$, so we want to show for all $a$, $b \in A_0$ that
$d(a,a,b) = b$.
Fix $a$, $b \in A_0$. By definition of $d$,
we have $d(a,a,b) = t(p(a,a,b), p(a,a,b), b) =b$.
The last equality holds since $t$ is a global-local difference term for $T$, in particular,
for $(A_0, 1)$.
...
have indices $j$ satisfying $k<j\leq n$ and $\chi_j = 1$.
For these we want to prove for all $a, b\in a_j$ that $d(a,a,b) = b$.
Fix $a, b\in a_j$. Since $p$ is a global-local difference term for $S'$, we have
$p(a,a,b) = b$, and this along with idempotence of $t$ yields
\[
d(a,a,b) =
t(p(a,a,b), p(a,a,b), b)
=t(b, b, b) =b\]
...
\[
\sS(\sA) =\Sub(\sA) \times \{0,1\},
\]
so $(A, i)\in \sS(\sA)$ indicates that
...
\begin{cor}
  ...
  algebra in $\sA$ if and only if each pair
  $((A,i), (B,i)) \in \sS(\sA)^2$ has a global-local
  difference term.
\end{cor}
\begin{proof}
  ...
  $\sS(\sA)$
  and this is a difference term for all of $\sA$.
  To see this, let $d$ be a global-local difference term for $\sS(\sA)$ and fix
  $\bA \in \sA$. Then for all  $a, a' \in A$
  we have $a \comm{\Cg(a,a')}{\Cg(a,a')} d(a,a',a')$,
  since $d$ is a global-local difference term for $(A,0)$,
  and for all  $a, a' \in A$ we have 
  $d(a,a,a') = a'$, since $d$ is a global-local difference term for $(A,1)$.
  Since $a, a'$ were arbitrary elements of $A$, we conclude that
...
\section{Local and global-local minority terms}
...
of $\sV$. We call $t$ a \defn{local minority term} for the triple
...
\end{align}
If $\sA$ is a collection of algebras in a variety $\sV$, and $t$ is a term of
$\sV$, then we call $t$ a \defn{global-local minority term} for 
$(A,i) \in \sA \times \{0,1,2\}$ provided for all $a, b \in A$ the
conditions~\ref{eq:2},~\ref{eq:4}, and~\ref{eq:5} hold. 

Suppose
$S = ((A_0, \chi_0), (A_1, \chi_1), \dots, (A_{n-1}, \chi_{n-1})$
is a sequence of pairs $(A_i, \chi_i) \in \sA \times \{0,1,2\}$.
We call a term $t$ a local minority term for $S$


\section{Global-local minority terms}
Let $\sV$ be a variety.  A term $m$ is called a minority term for $\sV$ if for
every $\bA \in \sV$ and all $a, b \in A$ it is the case that
\[
m(a,b,b) =m(b,a,b) = m(b,b,a) = a.
\]
Let $\bA = \<A, \dots\>$ be an algebra, let $t$ be a term in the language of
$\bA$, and let $i\in \{0,1,2\}$.
We call $t$ a \defn{local minority term} for $(A, i)$ provided for all
$a, b\in A$ we have
\begin{align*}
\text{ if $i=0$, then } & t(a, b, b)= a\\
\text{ if $i=1$, then } & t(b,a,b) = a\\
\text{ if $i=2$, then } & t(b,b,a) = a. 
\end{align*}
Let $\sA$ be a collection of algebras and suppose
\[S = ((A_0, \chi_0), (A_1, \chi_1), \dots, (A_{n-1}, \chi_{n-1})
\]
is a sequence of pairs $(A_i, \chi_i) \in \sA \times \{0,1,2\}$.
We call a term $t$ a local minority term for $S$
