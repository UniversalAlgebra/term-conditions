...
a ``local difference term,'' that we use, in Section~\ref{sec:algor-1:-exist},
...
Section~\ref{sec:algor-2:-exist-1}, we devise a polynomial-time algorithm for
...
\begin{proof}
  ...
  does not belong to $\com{\Cg^{\bA} (a, b)}$.
  But 
  $\com{\Cg^{\bB} (a, b)} \subseteq \com{\Cg^{\bA} (a, b)}$, so
  \[(
  d^{\bB}(a,b,b), a) \notin \com{\Cg^{\bB} (a, b)},
  \]
