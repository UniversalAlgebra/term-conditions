...
Elements of the disjoint union $\sU(A_0, A_1)$ may be denoted by
...
From now on we drop the ``mixed'' qualifier since, as far as the proofs go,
...
\begin{cor}
  ...
  $(\<(a_i,b_i,\chi_i),\iota_i\>,  \<(a_j,b_j,\chi_j),\iota_j\>)$ in the
  disjoint union $(\sum_{i \in I} (A_i^2\times \{0,1\})$ has a local difference term.
\end{cor}

\section{Glocal difference terms}
...
\begin{proof}
  ...
  $n^2$ pairs 
  $((A_i, \chi_i), (A_j, \chi_j)) \in \scS(\scA)^2$ has a \glocal
