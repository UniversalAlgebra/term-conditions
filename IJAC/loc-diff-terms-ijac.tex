...
\begin{proof}
  ...
  then the first projection is a local difference term. If $i = i' = 1$,  
    ...
    computing it that we describe in detail below. 
    (See Section~\ref{sec:interlude:-an-easy}.)
...
\section{Interlude: an easy route to the commutator}
...
\noindent {\bf Input:} A finite algebra, $\bA = \<A, \dots\>$, and two congruences $\alpha$, $\beta \in \Con \bA$.
...
\section{Local Difference Terms: Generalizations and Extensions}
...
\begin{proof}
  ...
  $\pi_A(\Com{\theta}) \subseteq \Com{\pi_A(\theta)}$
    ...
    $\pi_A(\theta) := \{(a,a') \in A^2 \mid ((a,b),(a',b')) \in \theta \text{ for
      some $(b,b')\in B^2$}\}$.}
  \[
    (d^{\bA}(a, a',a'), a)\in \pi_A\bigl(\Com{\Cg^{\bA \times \bB}((a, b), (a', b'))}\bigr)
