...
If $\sA$ be a collection of similar algebras. Let %% , we will use the notation
...
%% $A$ is a subuniverse of some algebra in $\sA$,and $i\in \{0,1\}$.
\[
\sS(\sA) =\sA \times \{0,1\},
\]
so $(A, i)\in \sS(\sA)$ indicates that
$A \in \sA$ and $i\in \{0,1\}$.
\begin{cor}
  ...
  %% subalgebra of every algebra in $\sA$
  algebra in $\sA$
  if and only if each pair $((A,i), (B,j)) \in \sS(\sA)^2$ has a global-local
  difference term.
\end{cor}
\begin{proof}
  ...
  $\sS(\sA)$
  and this is a difference term for every algebra in $\sA$.
  To see this, let $d$ be a global-local difference term for $\sS(\sA)$ and fix
  $\bA \in \sA$.  Then $d$ is a difference term operation for $\bA$.  Indeed,
  if $a, a' \in A$ we have $a \comm{\Cg(a,a')}{\Cg(a,a')} d(a,a',a')$,
  since $d$ is a global-local difference term for $(A,0)$,
  and we have $d(a,a,a') = a'$, since $d$ is a global-local difference term for $(A,1)$.
...
\begin{proof}
  ...
  Let $\sA = \{A_0, A_1, \dots, A_{n-1}\}$ and $\sS(\sA) = \sA \times \{0,1\}$.
  We first check that each of the
  %% $\binom{n}{2}$
  %% $\begin{pmatrix} n \\ 2 \end{pmatrix}$
  $n^2$ pairs
  $((A_i, \chi_i), (A_j, \chi_j)) \in (\sS \times \{0,1\})^2$ has a global-local
  difference term.
\end{proof}
...
global-local minority term for every $(A_i, \chi_i)$ in the sequence
$S$.
